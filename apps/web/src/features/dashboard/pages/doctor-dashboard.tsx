    ...
    <div className="dash-root dd-v2-grid">
      ...
      <div className="dd-v2-bottom-row">
        ...
        <div className="dd-list-card">
          ...
          <div>
            {todayAppts.filter(a => (['Waitlist', 'Consultation', 'Absent', 'Pending', 'Confirmed'].includes(a.status))).length > 0 ? (
              <div style={{ maxHeight: 280, overflowY: 'auto', paddingRight: 4 }} className="db-scroll">
                {todayAppts.filter(a => (['Waitlist', 'Consultation', 'Absent', 'Pending', 'Confirmed'].includes(a.status))).map(a => {
