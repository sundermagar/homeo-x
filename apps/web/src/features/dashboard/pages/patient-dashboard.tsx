...
import { 
  ...
  AlertCircle, AlertTriangle, Sparkles, Scale, Info, Gift, LogOut, ArrowLeft, MonitorSmartphone
} from 'lucide-react';
...
export function PatientDashboard() {
  ...
  const { data: apptsResult, isLoading: apptsLoading, refetch: refetchAppts } = useAppointments({ refetchInterval: 10000 });
  ...
  const isBirthdayToday = useMemo(() => {
    const dobStr = records?.medicalCase?.dateOfBirth;
  ...
  }, [records?.medicalCase?.dateOfBirth]);
  ...
  return (
    <div className="px-4 pt-2 pb-6 md:px-8 md:pt-0 md:pb-8 max-w-7xl mx-auto space-y-6 fade-in text-slate-700">

      {/* Birthday Banner — Premium Redesign */}
      {isBirthdayToday && (() => {
        // Generate star field (30 dots)
        const stars = Array.from({ length: 30 }).map((_, i) => {
          ...
          return (
            <div key={`star-${i}`} style={{
              ...
              animation: `starPulse ${dur} ease-in-out ${delay} infinite alternate`,
              zIndex: 1,
            }} />
          );
        });
        // Generate confetti ribbons (16 shapes)
        const RIBBON_COLORS = [
          'rgba(253,230,138,0.6)', 'rgba(167,139,250,0.6)',
          'rgba(129,210,255,0.5)', 'rgba(255,200,150,0.5)',
        ];
        const ribbons = Array.from({ length: 16 }).map((_, i) => {
          ...
          return (
            <div key={`ribbon-${i}`} style={{
              position: 'absolute', top: '-10px', left,
              width: isCircle ? w + 'px' : w + 'px',
              height: isCircle ? w + 'px' : h + 'px',
              borderRadius: isCircle ? '50%' : '1px',
              background: color,
              animation: `ribbonDrift ${dur} linear ${delay} infinite`,
              zIndex: 1,
            }} />
          );
        });
        ...
        return (
          <div 
            className="relative overflow-hidden text-white p-5 md:p-6 lg:p-7"
            style={{
            background: 'linear-gradient(110deg, #1a1f6e 0%, #3a2d9e 25%, #5b3ec8 50%, #7c4fd4 75%, #9b5de5 100%)',
            borderRadius: '18px',
            ...
            <div style={{
              ...
              background: 'rgba(180,140,255,0.25)', filter: 'blur(50px)',
              pointerEvents: 'none', zIndex: 0,
            }} />
            <div style={{
              position: 'absolute', bottom: '-60px', right: '80px',
              width: '180px', height: '180px', borderRadius: '50%',
              background: 'rgba(100,200,255,0.15)', filter: 'blur(50px)',
              pointerEvents: 'none', zIndex: 0,
            }} />
            ...
            {/* Main content */}
            <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-5 md:gap-4">
              <div className="flex items-start md:items-center gap-3 md:gap-4">
                {/* Cake icon — frosted glass */}
                <div className="w-12 h-12 md:w-14 md:h-14 rounded-xl bg-white/15 border border-white/25 flex items-center justify-center backdrop-blur-md shrink-0">
                  <span className="text-2xl md:text-3xl">🎂</span>
                </div>
                <div>
                  <h2 className="text-white text-lg md:text-2xl font-extrabold m-0 tracking-tight flex items-center flex-wrap gap-1.5">
                    Happy Birthday, {user?.name?.split(' ')[0] || 'Patient'}!
                    {/* Twinkling sparkles */}
                    <span style={{ color: '#fde68a', animation: 'bdayTwinkle 1.8s ease-in-out 0s infinite', display: 'inline-block' }} className="text-sm md:text-base">✦</span>
                    <span style={{ color: '#fde68a', animation: 'bdayTwinkle 1.8s ease-in-out 0.4s infinite', display: 'inline-block' }} className="text-xs md:text-sm">✧</span>
                    <span style={{ color: '#fde68a', animation: 'bdayTwinkle 1.8s ease-in-out 0.8s infinite', display: 'inline-block' }} className="text-sm md:text-base">✦</span>
                  </h2>
                  <p className="mt-1 md:mt-1.5 text-xs md:text-sm text-indigo-100/90 max-w-sm leading-relaxed mb-0">
                    Wishing you a year of great health, joy, and peace from the clinic.
                  </p>
                </div>
              </div>
            </div>
      ...
      {/* Active Call Banner */}
      {nextAppt && nextAppt.status === 'Consultation' && (
        <div className="relative rounded-2xl md:rounded-3xl p-6 md:p-8 text-white shadow-2xl overflow-hidden bg-gradient-to-r from-emerald-600 to-teal-500 animate-pulse-slow mb-6">
          <div className="absolute top-0 right-0 w-2/3 h-full bg-gradient-to-l from-white/20 to-transparent pointer-events-none" />
          <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="flex items-center gap-4">
              <div className="w-16 h-16 bg-white/20 rounded-full flex items-center justify-center backdrop-blur-md">
                <MonitorSmartphone size={32} className="text-white animate-bounce" />
              </div>
              <div>
                ...
                <p className="text-emerald-50 text-sm max-w-md">
                  Dr. {nextAppt.doctorName || 'Practitioner'} has started your session. Join the secure consultation room now.
                </p>
              </div>
            </div>
            <button
              onClick={() => {
                const visitType = nextAppt.visitType?.toUpperCase() || '';
                const notes = nextAppt.notes?.toUpperCase() || '';
                const isAudio = visitType === 'AUDIO' || notes.includes('AUDIO');
                const mode = isAudio ? 'AUDIO' : 'VIDEO';
                navigate(`/meet/${nextAppt.id}?mode=${mode}`);
              }}
              className="w-full md:w-auto px-8 py-4 bg-white text-emerald-600 rounded-xl font-bold text-lg hover:bg-emerald-50 hover:scale-105 transition-all shadow-xl flex items-center justify-center gap-2"
            >
              <Sparkles size={20} />
              Join Call
            </button>
          ...
          {/* Premium Hero Banner */}
          <div className="relative rounded-2xl md:rounded-3xl p-4 md:p-5 lg:p-6 text-slate-800 shadow-sm border border-slate-200 overflow-hidden bg-white">
            <div className="absolute top-0 right-0 w-2/3 h-full bg-gradient-to-l from-indigo-50/50 to-transparent pointer-events-none" />
            <div className="absolute -top-24 -right-10 w-96 h-96 bg-sky-100 rounded-full mix-blend-multiply filter blur-3xl opacity-40" />
            <div className="absolute -bottom-24 -right-32 w-96 h-96 bg-purple-100 rounded-full mix-blend-multiply filter blur-3xl opacity-40" />
            
            <div className="relative z-10 flex flex-col gap-3 md:gap-4">
              <div className="flex justify-between items-start">
                <div>
                  <h1 className="text-xl md:text-2xl lg:text-3xl font-extrabold tracking-tight mb-1">
                    Welcome back,{' '}
                    <span 
                  ...
                  <p className="text-[11px] md:text-xs text-slate-500 font-medium max-w-xl leading-relaxed m-0">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2.5 p-3 rounded-xl bg-slate-50/80 backdrop-blur-md border border-slate-100 mt-2">
                <div className="space-y-0.5">
                  <span className="text-slate-400 text-[9px] md:text-[10px] uppercase font-bold tracking-wider">Patient ID</span>
                  <p className="font-mono font-bold text-slate-700 text-xs md:text-sm m-0">PT-{user?.regid || user?.id || '—'}</p>
                </div>
                <div className="space-y-0.5">
                  <span className="text-slate-400 text-[9px] md:text-[10px] uppercase font-bold tracking-wider">Age / Gender</span>
                  <p className="font-bold text-slate-700 text-xs md:text-sm m-0">
                    {age ? `${age} yrs` : '—'} • {medicalCase?.gender || '—'}
                  </p>
                </div>
                <div className="space-y-0.5">
                  <span className="text-slate-400 text-[9px] md:text-[10px] uppercase font-bold tracking-wider">Primary Doctor</span>
                  <p className="font-bold text-slate-700 text-xs md:text-sm truncate m-0">
                    {medicalCase?.doctorName || 'Practitioner'}
                  </p>
                </div>
                <div className="space-y-0.5">
                  <span className="text-slate-400 text-[9px] md:text-[10px] uppercase font-bold tracking-wider">Registered On</span>
                  <p className="font-bold text-slate-700 text-xs md:text-sm m-0">
