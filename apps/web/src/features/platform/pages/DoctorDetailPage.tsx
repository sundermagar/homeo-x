  ...
  return (
    <div className="plat-profile-page animate-fade-in">
      ...
      <div className="plat-profile-header">
        <div className="plat-profile-container h-14 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="flex flex-col">
               <span className="text-[9px] font-black text-[var(--pp-text-3)] uppercase tracking-[0.2em] leading-none mb-1">Electronic Clinical Record</span>
               <h2 className="text-xs font-black text-[var(--pp-ink)] uppercase tracking-widest leading-none">Practitioner ID # {String(id).padStart(4, '0')}</h2>
            </div>
          </div>
          <div className="flex items-center gap-4">
             <div className="flex items-center gap-2 px-3 py-1 bg-[var(--pp-warm-2)] border border-[var(--pp-warm-4)] rounded-full">
                <span className={`w-1.5 h-1.5 rounded-full ${staff.isActive ? 'bg-[var(--pp-success-fg)] animate-pulse' : 'bg-[var(--pp-text-4)]'}`} />
                <span className="text-[10px] font-black text-[var(--pp-ink)] uppercase tracking-wider">{staff.isActive ? 'Active' : 'Inactive'}</span>
             </div>
          </div>
        </div>
      </div>
          ...
          <aside className="plat-sidebar space-y-6">
            <div className="plat-profile-card text-center">
               <div className="inline-block relative mb-6">
                  <div className="w-28 h-28 rounded-[24px] bg-[var(--pp-warm-2)] border-2 border-white shadow-[var(--pp-shadow-sm)] relative overflow-hidden flex items-center justify-center">
                    {staff.profilepic ? (
                      <img src={staff.profilepic} alt={staff.name} className="w-full h-full object-cover" />
                    ) : (
                      <User size={40} className="text-[var(--pp-blue-border)]" />
                    )}
                  </div>
               </div>
               ...
               <div className="inline-flex items-center gap-2 px-3 py-1 bg-[var(--pp-blue-tint)] text-[var(--pp-blue)] rounded-full">
                  <Award size={12} />
                  <span className="text-[10px] font-black uppercase tracking-wider plat-capitalize">{staff.designation || 'Specialist Consultant'}</span>
               </div>
               ...
               <div className="mt-8 pt-8 border-t border-[var(--pp-warm-2)] grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <span className="plat-profile-label">Institutional Tenure</span>
                    <p className="plat-profile-value text-lg">{staff.joiningdate ? new Date(staff.joiningdate).getFullYear() : '—'}</p>
                  </div>
               </div>
            </div>
            ...
            <div className="plat-profile-card">
               <h3 className="plat-profile-section-title !mb-4">
                  <div className="w-6 h-6 bg-[var(--pp-blue-tint)] text-[var(--pp-blue)] rounded flex items-center justify-center"><Phone size={12} /></div>
                  Contact Gateway
               </h3>
               <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs font-bold text-[var(--pp-ink)] truncate">{staff.email}</p>
                  </div>
                  <div>
          ...
          <section className="plat-main space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
               <div className="plat-profile-card">
                  <h3 className="plat-profile-section-title">
                    <div className="w-6 h-6 bg-[var(--pp-success-bg)] text-[var(--pp-success-fg)] rounded flex items-center justify-center"><GraduationCap size={12} /></div>
                    Academic Record
                  </h3>
                  <div className="space-y-4">
                     <div className="p-4 bg-[var(--pp-warm-2)] rounded-xl border border-white">
                     ...
                     <div className="flex items-center gap-2 px-3 py-1.5 bg-[var(--pp-blue-tint)] border border-[var(--pp-blue-border)] rounded-lg w-max">
                        <Landmark size={12} className="text-[var(--pp-blue)]" />
                        <span className="text-[10px] font-black text-[var(--pp-blue-deep)] font-mono tracking-tighter uppercase">REG_ID: {staff.registrationId || 'PENDING'}</span>
                     </div>
                  </div>
               </div>
               ...
               <div className="plat-profile-card">
                  <h3 className="plat-profile-section-title">
                    <div className="w-6 h-6 bg-amber-50 text-amber-600 rounded flex items-center justify-center"><Fingerprint size={12} /></div>
                    Identity & Compliance
                  </h3>
                  <div className="space-y-4">
                     <div className="flex flex-col p-4 bg-[var(--pp-warm-1)] rounded-xl border border-[var(--pp-warm-4)]">
            ...
            <div className="plat-profile-card">
               <div className="flex items-center justify-between mb-6">
                  <h3 className="plat-profile-section-title !mb-0">
                    <div className="w-6 h-6 bg-purple-50 text-purple-600 rounded flex items-center justify-center"><Files size={12} /></div>
                    Digital Credential Vault
                  </h3>
                  <div className="flex items-center gap-1.5 px-2 py-0.5 bg-purple-50 border border-purple-100 rounded text-[9px] font-black text-purple-700 uppercase">
                    <Sparkles size={8} /> Institutional Compliance
                  </div>
               </div>
               
               <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {renderDocument('Medical Registration', staff.registrationCertificate)}
                  {renderDocument('Identity Document', staff.aadharCard)}
                  {renderDocument('Taxation Identity', staff.panCard)}
                  {renderDocument('Clinical Degree: BHMS', staff.bhmsDocument)}
                  {renderDocument('Advanced Degree: MD', staff.mdDocument)}
                  {renderDocument('Secondary Certificate', staff.col12Document)}
                  {renderDocument('Primary Certificate', staff.col10Document)}
                  {renderDocument('Employment Record', staff.appointmentLetter)}
               </div>
            </div>
            ...
            <div className="plat-profile-card">
               <h3 className="plat-profile-section-title !mb-6">
                  <div className="w-6 h-6 bg-[var(--pp-danger-bg)] text-[var(--pp-danger-fg)] rounded flex items-center justify-center"><MapPin size={12} /></div>
                  Residency Log
               </h3>
               <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div>
                    <span className="plat-profile-label">Primary Station</span>
                    <p className="text-[13px] font-medium leading-relaxed bg-[var(--pp-warm-2)] p-4 rounded-xl min-h-[70px] plat-capitalize">
                      {staff.address || 'Administrative station not recorded.'}
                    </p>
                  </div>
                  <div>
                    <span className="plat-profile-label">Permanent Record</span>
                    <p className="text-[13px] font-medium leading-relaxed border border-[var(--pp-warm-2)] p-4 rounded-xl min-h-[70px] plat-capitalize">
            ...
            <div className="plat-profile-card">
               ...
               <h3 className="plat-profile-section-title">
                  <div className="w-6 h-6 bg-[var(--pp-blue-tint)] text-[var(--pp-blue)] rounded flex items-center justify-center"><FileText size={12} /></div>
                  Institutional Summary
               </h3>
               <p className="text-sm leading-relaxed text-[var(--pp-text-2)] font-medium max-w-2xl relative z-10">
                  {staff.about || 'A professional practitioner summary is currently being prepared for this clinical record. All academic and institutional credentials have been digitally verified for regulatory compliance.'}
               </p>
            </div>
          </section>
        </div>
