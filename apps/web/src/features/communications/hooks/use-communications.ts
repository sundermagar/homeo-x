...
export function useSmsTemplates() {
  return useQuery({
    ...
    queryFn: () =>
      apiClient
        .get<{ success: boolean; data: SmsTemplate[] }>('/communications/templates')
        .then((r) => r.data.data ?? []),
  });
}
...
export function useCreateTemplate() {
  ...
  return useMutation({
    mutationFn: (dto: CreateSmsTemplateDto) => apiClient.post('/communications/templates', dto),
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return apiClient
        .get<{
          success: boolean;
          data: { data: SmsReport[]; total: number };
        }>(`/communications/reports?${params}`)
        .then((r) => r.data.data);
    },
    staleTime: 30_000,
  });
...
export function useWhatsAppLogs() {
  return useQuery({
    ...
    queryFn: () =>
      apiClient.get('/communications/whatsapp/logs').then((r) => (r.data as any).data ?? []),
    staleTime: 60_000,
  });
}
