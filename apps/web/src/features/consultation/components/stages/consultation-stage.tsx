...
> = {
  acute: {
    ...
    desc: 'Recent onset — focus on onset, modalities, sensation',
  },
  chronic: {
    ...
    desc: 'Long-standing — constitutional analysis, life events',
  },
  followup: {
    ...
    desc: 'Review response to previous prescription',
  },
};
...
export function ConsultationStage({
  ...
  const patientAnswerSegmentsRef = useRef<
    { text: string; translatedText: string; timestamp: number }[]
  >([]);
  ...
  useEffect(() => {
    ...
    if (isLiveKitMode) {
      ...
      // 1. Doctor voice
      if (
        !hasAutoStartedDr.current &&
        !binaryTranscriber.isRecording &&
      ...
      if (
        !hasAutoStartedPt.current &&
        !patientTranscriber.isRecording &&
        !patientTranscriber.isConnecting
      ) {
  ...
  const injectQuestion = useCallback(
    (questionText: string, options?: string[], questionId?: string) => {
      ...
      if (vcSocketRef.current?.connected) {
        vcSocketRef.current.emit('call:send-question', {
          visitId,
          question: questionText,
          options,
        });
  ...
  const injectAnswer = useCallback(
    (answerText: string, forQuestion?: string) => {
      ...
      if (forQuestion) {
        ...
        setAnsweredQuestions((prev) =>
          prev.includes(forQuestion) ? prev : [...prev, forQuestion],
        );
      }
      ...
      symptomExtraction.mutate(
        ...
        },
        {
          onSuccess: (result) => {
            ...
            if (
              result &&
              (result.mental?.length || result.physical?.length || result.particular?.length)
            ) {
              onSymptomsExtracted(result);
            }
          },
  ...
  const handleRemoveSymptom = useCallback(
    (category: 'mental' | 'physical' | 'particular', index: number) => {
    ...
    <div className="space-y-6 pp-fade-in relative">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-2">
        <div className="space-y-1">
          ...
          <p className="text-sm font-medium text-[#4A4A47]">
            Holistic Analysis & Symptom Extraction
          </p>
        </div>
        <span className="inline-flex items-center gap-2 px-3 py-1.5 rounded-md bg-[#EFF6FF] border border-[#BFDBFE] text-xs font-bold text-[#2563EB] tracking-wider uppercase w-fit">
          ...
          {MODE_CONFIG[consultationMode].label} <span className="opacity-50">|</span>{' '}
              ...
              symptomExtraction.mutate(
                ...
                },
                {
                  onSuccess: (result) => {
                    ...
                    if (
                      result &&
                      (result.mental?.length ||
                        result.physical?.length ||
                        result.particular?.length)
                    ) {
                      onSymptomsExtracted(result);
                    } else {
                      toast({
                        ...
                        description:
                          'Lab parsed but no rubrics were extracted. Check that values are flagged as high/low or out of range.',
                        variant: 'default',
                      });
                    }
                  },
          ...
          <div className="pp-card overflow-hidden">
            <div className="px-5 py-3 bg-[#FAFAF8] border-b border-[#E3E2DF] flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[14px] font-bold text-[#0F0F0E] tracking-tight">
            ...
            <div className="p-4 space-y-2 bg-white">
              {allQuestions.filter((q: any) => !q.answered).length === 0 &&
                !modeQuestions.isPending && (
                      ...
                      >
                        <Star
                          className={cn(
                            ...
                            q.isLive
                              ? 'text-amber-500'
                              : 'text-[#2563EB] opacity-70 group-hover:opacity-100',
                          )}
                        />
          ...
          <div className="pp-card sticky top-8">
            ...
            <div className="divide-y divide-[#E3E2DF] bg-white">
              <div className="p-4">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-[10px] font-bold text-[#4A4A47] uppercase tracking-widest">
                    Mental Generals
                  </span>
            ...
            <div className="px-4 py-2 bg-[#FAFAF8] border-t border-[#E3E2DF] text-center">
              <span className="text-[11px] font-bold text-[#888786] uppercase tracking-widest">
                {totalSymptoms} symptom{totalSymptoms !== 1 ? 's' : ''} extracted
              </span>
            </div>
          </div>
        </div>
