...
export function ConsultationPage() {
  ...
  const [ongoingTranscript, setOngoingTranscript] = useState<string>('');

  const rxForm = useForm<PrescriptionFormData>({
    defaultValues: { notes: '', items: [] },
  });

  const { append: appendMedication } = useFieldArray({
  ...
  const handleAutoSuggestRemedy = (remedyName: string, potencies: string[]) => {
    ...
    if (!alreadyExists) {
      appendMedication({
        ...
        duration: '1 day',
        route: 'Oral',
        instructions: `Potency: ${potencies[0] || '30C'}`,
      });
      toast({
        title: 'Remedy Suggested',
        description: `Added ${remedyName} to prescription based on repertorization.`,
        variant: 'default',
      });
    }
  };
