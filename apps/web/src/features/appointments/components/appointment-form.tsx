...
export function AppointmentForm({
  ...
  const [form, setForm] = useState({
    ...EMPTY_FORM,
    bookingDate: initialDate ?? EMPTY_FORM.bookingDate,
  });
  ...
  const isPatientRole = user?.type === Role.Patient;

  const { data: patientProfile } = usePatient(
    isPatientRole && user?.regid ? Number(user.regid) : 0
  );
  ...
  useEffect(() => {
    ...
    if (!editAppointment && user?.type === Role.Doctor && doctors.length > 0) {
      const myDoc = doctors.find((d: Doctor) => d.id === user.id);
      if (myDoc) {
        setForm((f) => ({
          ...f,
          doctorId: String(myDoc.id),
          consultationFee:
            myDoc.consultation_fee !== undefined && myDoc.consultation_fee !== null
              ? String(myDoc.consultation_fee)
              : f.consultationFee,
        }));
      }
    }
  }, [doctors, user, editAppointment]);
  ...
  // Auto-populate patient details if user is a Patient
  useEffect(() => {
    if (isPatientRole && user) {
      setForm((f) => ({
        ...f,
        patientId: String(user.regid ?? ''),
        patientName: user.name ?? '',
        phone: patientProfile?.mobile1 ?? patientProfile?.phone ?? f.phone,
        visitType: VisitType.FollowUp,
      }));
    }
  }, [isPatientRole, user, patientProfile]);

  const handleDoctorChange = (id: string) => {
    const doc = doctors.find((d: Doctor) => String(d.id) === id);
    setForm((f) => ({
      ...f,
      doctorId: id,
      consultationFee:
        doc?.consultation_fee !== undefined && doc?.consultation_fee !== null
      ...
      const timer = setTimeout(async () => {
        try {
          const { data } = await apiClient.get<any>(
            `/patients/lookup?query=${encodeURIComponent(value)}`,
          );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.notes || !form.notes.trim()) {
      setError(
        isPatientRole
          ? 'Reason for Visit / Symptoms is required — describe what brings you in today.'
          : 'Chief Complaint is required — describe what brings the patient in today.'
      );
      return;
    }
    ...
    return (
      <div className="appt-success-panel animate-fade-in">
        ...
        <div className="appt-success-text">
          ...
          <p>
            Dr. {bookingResult.doctorName} • {form.bookingDate} at {form.bookingTime}
          </p>
          {bookingResult.tokenNo && (
            <span className="appt-token-badge">Token #{bookingResult.tokenNo}</span>
          )}
        </div>
  ...
  return (
    <form onSubmit={handleSubmit} className="appt-form">
      {error && <div className="appt-alert appt-alert-error">{error}</div>}
        ...
        <div className="appt-form-group">
          ...
          {selectedDoctorInactive && (
            <div className="appt-offline-banner">
              ...
              <span>
                <strong>Dr. {doctors.find((d: Doctor) => String(d.id) === form.doctorId)?.name}</strong> is
                currently <strong>Inactive</strong>. Appointments cannot be booked.
              </span>
            </div>
          )}

          {/* Read-only fee display for Patient Portal */}
          {isPatientRole && form.doctorId && (
            <div className="appt-patient-fee-info animate-fade-in">
              <span>Consultation Fee: </span>
              <strong>{Number(form.consultationFee) > 0 ? `₹${form.consultationFee}` : 'Free / TBA'}</strong>
            </div>
          )}
        </div>
        {!isPatientRole && (
      ...
      <div className="appt-form-group">
        ...
        {selectedDoctorInactive ? (
          ...
          <div className="appt-slots-hint">Select doctor and date to see available slots</div>
        ) : (
      ...
      <div className="appt-form-group" style={{ position: 'relative' }}>
        <label className="appt-form-label">
          ...
          {isPatientRole ? 'Booking For' : 'Patient Selection'}
        </label>

        {isPatientRole ? (
          <div className="appt-patient-profile-card animate-fade-in">
            <div className="appt-patient-profile-avatar">
              <User size={18} />
            </div>
            <div className="appt-patient-profile-info">
              <h4 className="appt-patient-profile-name">{user?.name}</h4>
              <div className="appt-patient-profile-meta">
                <span>
                  <strong>Reg No:</strong> {user?.regid || 'N/A'}
                </span>
                {form.phone && (
                  <span>
                ...
                {suggestions.map((p) => (
                  ...
                  >
                    ...
                    <div className="appt-autocomplete-meta">
                      <span>
                        <strong>ID:</strong> {p.regid || p.id}
                      </span>
                      {(p.mobile1 || p.phone) && (
                        <span>
                          <Phone size={10} /> {p.mobile1 || p.phone}
                        </span>
                      )}
                      {p.lastVisit && (
              ...
              <div className="appt-autocomplete-list appt-autocomplete-empty">
                <p>
                  ...
                  {form.visitType === VisitType.FollowUp
                    ...
                    : 'You can enter a new patient name.'}
                </p>
              </div>
            )}
          </div>
        ...
        {!isPatientRole && form.visitType === VisitType.FollowUp && form.patientName && !showSuggestions && (
      ...
      <div className="appt-form-group">
        <label className="appt-form-label">
          {isPatientRole ? 'Reason for Visit / Symptoms' : 'Chief Complaint'}{' '}
          <span style={{ color: 'var(--pp-danger-fg)' }}>*</span>
        </label>
        <textarea
          className="appt-form-input appt-form-textarea"
          placeholder={
            isPatientRole
              ? 'What symptoms are you experiencing or what brings you in today? e.g. fever for 3 days, headache, cough...'
              : 'What brings the patient in today? e.g. fever for 3 days, recurring headache, anxiety…'
          }
          value={form.notes}
        ...
        <p style={{ fontSize: 11, color: 'var(--pp-text-3)', margin: '4px 0 0' }}>
          {isPatientRole
            ? 'Required. Describe your symptoms or reason for visit to help the doctor understand your condition.'
      ...
      <div className="appt-form-actions">
        ...
        <button
          ...
          title={
            selectedDoctorInactive
              ? 'Doctor is inactive. Please select an available doctor.'
              : undefined
          }
        >
          {isLoading ? (
