    ...
    <div className="appt-page">
      ...
      <div className="appt-header">
        ...
        <div className="appt-header-actions">
          <button
            onClick={() => {
              setBookingDate(selectedDay ?? todayISO);
      ...
      <div className="appt-cal-wrap">
        ...
        <div className="appt-card">
          ...
          <div className="appt-card-header">
            <div className="appt-cal-nav">
              <button className="appt-btn appt-btn-icon" onClick={prevMonth}>
                <ChevronLeft size={15} strokeWidth={1.6} />
              </button>
              <span className="appt-cal-month-label">
                {MONTHS[month]} {year}
              </span>
              <button className="appt-btn appt-btn-icon" onClick={nextMonth}>
          ...
          <div className="appt-cal-body">
            ...
            <div className="appt-cal-grid-header">
              {DAYS.map((d) => (
                <div key={d} className="appt-cal-dow">
                  {d}
                </div>
              ))}
            </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        <span
                          className={
                            isToday ? 'appt-cal-day-num appt-cal-day-today' : 'appt-cal-day-num'
                          }
                          style={{
                            fontSize: '0.82rem',
                            fontWeight: isToday ? 800 : 600,
                            color: isToday ? 'var(--pp-blue)' : 'var(--pp-ink)',
                          }}
                        >
                          {dNum}
                        </span>
                      ...
                      <div className="appt-cal-bars">
                        {dayAppts.slice(0, 3).map((a, i) => (
                          <div
                            key={i}
                            className="appt-cal-bar"
                            style={{ background: STATUS_COLOR[a.status] ?? 'var(--pp-warm-4)' }}
                          />
                        ))}
                      </div>
                      {dayAppts.length > 0 && (
                        <div className="appt-cal-count">
                          {dayAppts.length} slot{dayAppts.length > 1 ? 's' : ''}
                        </div>
                      )}
                    </div>
                  );
        ...
        <div className="appt-side-panel">
          ...
          <div className="appt-card">
            <div className="appt-card-header">
              <h3 className="appt-card-title">
                ...
                {selectedDay
                  ? new Date(selectedDay + 'T12:00').toLocaleDateString('en-GB', {
                      weekday: 'short',
                      day: 'numeric',
                      month: 'short',
                    })
              ...
              {isLoading ? (
                <div className="appt-slot-list">
                  {[...Array(3)].map((_, i) => (
                    <div
                      key={i}
                      className="appt-slot-item appt-shimmer"
                      style={{ height: 62, border: 'none' }}
                    />
                  ))}
                </div>
              ) : !selectedDay ? (
                    ...
                    {selectedAppts.map((a) => (
                      <div key={a.id} className="appt-slot-item">
                        ...
                        <div className="appt-slot-name">
                          <div className="appt-slot-patient">
          ...
          <div className="appt-card">
            <div className="appt-card-header">
              <h3 className="appt-card-title">
                <Stethoscope size={14} strokeWidth={1.6} className="appt-panel-title-icon" />{' '}
                Insights
              </h3>
            </div>
            <div className="appt-card-body">
              {isLoading ? (
                <div className="appt-insights-grid">
                  {[...Array(2)].map((_, i) => (
                    <div
                      key={i}
                      className="appt-insight-item appt-shimmer"
                  ...
                  {(
                    [
                      ...
                      },
                      {
                        ...
                        value: `${daysInMonth > 0 ? Math.round((totalAppts / daysInMonth) * 10) / 10 : 0} / day`,
                        icon: <Users size={15} strokeWidth={1.6} />,
                        bg: 'var(--pp-success-bg)',
                        ic: 'var(--pp-success-fg)',
                      },
                    ] as const
                  ).map((item) => (
                    <div key={item.label} className="appt-insight-item">
                      <div
                        className="appt-insight-icon-wrap"
