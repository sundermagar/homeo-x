    ...
    sendText.mutate(
      ...
      },
      {
        onSuccess: () =>
          toast({
            title: '✅ WhatsApp Sent',
            description: `Expiry reminder sent to ${rec.firstName}.`,
          }),
        onError: (err: any) =>
          toast({
            title: '❌ Send Failed',
  ...
  return (
    <div className="pp-page-container pkg-page animate-fade-in">
      ...
      {selectedIds.size > 0 && (
        ...
        >
          <span style={{ fontWeight: 700, fontSize: '0.85rem', color: 'var(--pp-blue)' }}>
      ...
      <div className="pkg-stats">
        {[
          ...
          },
          {
            ...
            color: '#E11D48',
            bg: '#FFF1F2',
            icon: <XCircle size={20} strokeWidth={1.6} />,
          },
        ].map((s) => (
          <div key={s.label} className="pkg-stat-card">
            <div className="pkg-stat-icon" style={{ background: s.bg, color: s.color }}>
              {s.icon}
            </div>
            <div>
                ...
                <tbody>
                  {paginatedData.map((r: any, i: number) => (
                    <tr
                      ...
                      style={{
                        cursor: 'pointer',
                        background: selectedIds.has(Number(r.regid))
                          ? 'var(--pp-blue-tint)'
                          : undefined,
                      }}
                    ...
                    >
                      ...
                      >
                        <div className="flex justify-end md:justify-start">
                          ...
                          >
                            {selectedIds.has(Number(r.regid)) ? (
                              <CheckSquare size={16} style={{ color: 'var(--pp-blue)' }} />
                            ) : (
                              <Square size={16} style={{ color: 'var(--pp-text-3)' }} />
                            )}
                      ...
                      <td data-label="PATIENT" className="pkg-patient-cell">
                        ...
                        <div className="flex flex-col items-end md:items-start text-right md:text-left">
                          <div className="pkg-patient-name" style={{ fontWeight: 700 }}>
                            {r.firstName} {r.surname}
                          </div>
                          {r.phone && (
                            <div
                              className="pkg-patient-phone"
                              style={{
                                fontSize: '0.72rem',
                      ...
                      <td data-label="PACKAGE" className="pkg-plan-cell">
                        <div className="flex flex-col items-end md:items-start text-right md:text-left">
                          <div className="pkg-plan-val" style={{ fontWeight: 600 }}>
                            {r.packageName}
                          </div>
                      ...
                      <td data-label="START" style={{ fontSize: '0.82rem' }}>
                        {r.startDate}
                      </td>
                      <td
                        data-label="EXPIRY"
                        style={{ fontSize: '0.82rem', fontWeight: 600, color: 'var(--pp-ink)' }}
                      >
                        {r.expiryDate}
                      </td>
        ...
        {selectedRecord && (
          <div className="pkg-drawer-details">
            ...
            >
              ...
              >
                {selectedRecord.firstName?.[0]}
                {selectedRecord.surname?.[0]}
              </div>
              <div>
                <h3 style={{ margin: 0, fontSize: '1.1rem', fontWeight: 800 }}>
                  {selectedRecord.firstName} {selectedRecord.surname}
                </h3>
                <p style={{ margin: 0, color: 'var(--pp-text-3)', fontSize: '0.85rem' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              <section>
                ...
                <div
                  style={{ padding: 16, border: '1.5px solid var(--pp-warm-4)', borderRadius: 12 }}
                >
                  <div style={{ fontWeight: 700, fontSize: '1rem', color: 'var(--pp-blue)' }}>
                    {selectedRecord.packageName}
                  </div>
                  <div style={{ fontSize: '0.85rem', color: 'var(--pp-text-3)', marginTop: 4 }}>
                    Price: ₹{selectedRecord.packagePrice?.toLocaleString()}
                  </div>
                </div>
              </section>
              ...
              <section>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <span
                    className={`pkg-expiry-badge ${getStatusBadgeClass(selectedRecord.status)}`}
              ...
              <section style={{ marginTop: 12 }}>
                ...
                >
                  ...
                  <span style={{ fontWeight: 600 }}>
                    {selectedRecord.phone || 'No phone provided'}
                  </span>
                </div>
              </section>
              ...
              <hr
                style={{
                  margin: '8px 0',
                  border: 'none',
                  borderTop: '1px dashed var(--pp-warm-4)',
                }}
              />
            ...
            >
              <button className="pp-btn pp-btn-secondary" onClick={() => setSelectedRecord(null)}>
                Close
              </button>
              <button
              ...
              <button
                className="pp-btn pp-btn-primary"
                onClick={() => (window.location.href = `/patients/${selectedRecord.patientId}`)}
              >
        ...
        >
          ...
          >
            <h3 style={{ margin: '0 0 16px 0', display: 'flex', alignItems: 'center', gap: 8 }}>
              <MessageCircle size={18} style={{ color: '#25D366' }} /> Send WhatsApp (
              {selectedIds.size} patients)
            </h3>
            <div style={{ marginBottom: 16 }}>
              ...
              >
                <p style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', margin: 0 }}>
                  <strong>Template Preview:</strong> Dear <strong>{'{name}'}</strong> Your package
                  expires on <strong>{'{date}'}</strong>. Kindly call on 8727001444 to renew it.
                  Ignore if already renewed. Regards MMC HomeoTech
                </p>
              </div>
            </div>
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
              <button className="pp-btn pp-btn-secondary" onClick={() => setShowSmsModal(false)}>
                Cancel
              </button>
              <button
                className="pp-btn"
