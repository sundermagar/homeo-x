...
export const CampaignModal = ({ isOpen, onClose }: CampaignModalProps) => {
  const { useChannels, useTemplates, useCreateCampaign, useUploadMedia, useContacts } =
    useWhatsApp();
  ...
  const { data: templates } = useTemplates(
    selectedChannelId ? Number(selectedChannelId) : undefined,
  );
  ...
  const templateVariables = selectedTemplate
    ? selectedTemplate.body.match(/\{\{\d+\}\}/g) || []
    : [];
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      if (rows.length < 2) {
        toast({
          title: 'Invalid CSV',
          description: 'File must contain headers and at least one recipient.',
          variant: 'error',
        });
        return;
      }
      ...
      toast({
        title: 'Matrix Loaded',
        description: `Detected ${data.length} recipients and ${headers.length} columns.`,
      });
    };
  ...
  const onSubmit = (data: CampaignFormInput) => {
    if (csvRecipients.length === 0) {
      toast({
        ...
        description: 'Please upload a recipient CSV to proceed.',
        variant: 'error',
      });
      return;
    }
    ...
    if (!mapping['phone']) {
      toast({
        title: 'Mapping Error',
        description: 'You must map the "Phone" column.',
        variant: 'error',
      });
      return;
    }
    ...
    if (template?.mediaType !== 'text' && !headerMediaId) {
      toast({
        ...
        description: `This template requires a ${template?.mediaType || 'media'} header.`,
        variant: 'error',
      });
      return;
    }
    ...
    createCampaignMutation.mutate(
      {
        ...
        apiType: 'cloud_api',
        type:
          selectedTemplate?.category?.toLowerCase() === 'utility' ? 'transactional' : 'marketing',
      ...
      },
      {
        onSuccess: () => {
          toast({
            ...
            description: `${recipients.length} recipients synchronized successfully.`,
          });
          reset();
          setCsvRecipients([]);
          setCsvHeaders([]);
  ...
  const handleMediaUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    uploadMediaMutation.mutate(
      { channelId: Number(selectedChannelId), file },
      {
        onSuccess: (res: any) => {
        ...
        <div className="appt-drawer-body">
          <form onSubmit={handleSubmit(onSubmit)} className="appt-form">
            ...
            <div className="appt-form-group">
              ...
              >
                <option value="" disabled>
                  {selectedChannelId ? 'Choose broadcast template' : 'Select a channel first'}
                </option>
                {templates?.map((t: any) => (
                  ...
                  {csvRecipients.length > 0 ? (
                    <div className="flex flex-col items-center">
                      ...
                      <span className="text-[13px] font-bold text-main">
                        {csvRecipients.length} Patients Prepared
                      </span>
                      <span className="text-[10px] text-muted font-medium mt-1">
                        Click to replace matrix
                      </span>
                    </div>
                  ) : (
                    <div className="flex flex-col items-center">
                      ...
                      <span className="text-[10px] text-muted font-medium mt-1">
                        Columns must include Phone numbers
                      </span>
                    </div>
                  )}
                </div>
                ...
                <div className="max-h-48 overflow-y-auto border border-pp-border rounded-xl p-3 space-y-2 bg-white">
                  {filteredContacts.length === 0 ? (
                    ...
                    filteredContacts.map((contact: any) => {
                      const isChecked = selectedDbContacts.some(
                        (c: any) => c.phone === contact.phone,
                      );
                      return (
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              if (e.target.checked) {
                                ...
                                setSelectedDbContacts(
                                  selectedDbContacts.filter((c: any) => c.phone !== contact.phone),
                                );
                              }
                            }}
                          ...
                          {contact.tags && contact.tags.length > 0 && (
                            <div className="flex gap-1 flex-wrap max-w-[120px] justify-end">
                              {contact.tags.map((tag: string) => (
                                <span
                                  key={tag}
                                  className="text-[8px] font-bold uppercase tracking-wider bg-pp-bg-subtle text-pp-blue px-1.5 py-0.5 rounded"
                                >
                                  {tag}
                                </span>
                              ))}
                ...
                <div className="p-5 bg-pp-bg-subtle/50 rounded-xl border border-pp-border space-y-4">
                  ...
                  {templateVariables.map((_, i) => (
                    <div key={i} className="flex items-center justify-between">
                      <span className="text-[11px] font-bold text-pp-blue uppercase tracking-wider">
                        Variable &#123;&#123;{i + 1}&#125;&#125;
                      </span>
                      <select
                        className="appt-form-select h-8 py-0 px-2 text-[11px] w-40"
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-[12px] font-bold text-main">
                        {headerMediaId ? 'Media Linked' : `Select ${selectedTemplate.mediaType}`}
                      </p>
                      <p className="text-[10px] text-muted font-medium">
                        {headerMediaId
                          ? 'Securely stored on Meta Cloud'
                          : 'Required for this template'}
                      </p>
                    </div>
                  </div>
                  {uploadMediaMutation.isPending && (
                    <Loader2 size={16} className="animate-spin text-pp-blue" />
                  )}
                  {headerMediaId && !uploadMediaMutation.isPending && (
