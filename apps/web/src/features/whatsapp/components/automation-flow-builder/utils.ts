...
export const uid = () => `node_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
...
export function transformAutomationToFlow(automation: any): {
  ...
  const flowNodes = automation?.flowData?.nodes || automation?.automation_nodes;
  const flowEdges = automation?.flowData?.edges || automation?.automation_edges;

  if (!automation || !flowNodes) {
    return {
      nodes: [
        {
          id: 'start',
  ...
  const sortedNodes = [...flowNodes].sort((a: any, b: any) => a.position - b.position);
  ...
  sortedNodes.forEach((autoNode: any, index: number) => {
    ...
    const reactFlowNode: Node<BuilderNodeData> = {
      id: autoNode.nodeId || autoNode.id,
      type: autoNode.type,
      position:
        autoNode.position && autoNode.position.x !== undefined && autoNode.position.y !== undefined
  ...
  if (flowEdges && flowEdges.length > 0) {
    ...
    sortedNodes.forEach((autoNode: any) => {
      const nodeId = autoNode.nodeId || autoNode.id;
