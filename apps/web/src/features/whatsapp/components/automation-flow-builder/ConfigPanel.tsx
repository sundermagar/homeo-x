...
function SectionHeader({ children }: { children: React.ReactNode }) {
  return (
    <div className="text-[10px] font-bold uppercase tracking-[0.15em] text-gray-400 mt-1">
      {children}
    </div>
  );
}
...
export function ConfigPanel({
  ...
  const { data: contactGroups = [] } = useQuery({
    queryKey: ['whatsapp', 'groups', channelId],
    queryFn: async () => {
      if (!channelId) return [];
      const { data } = await apiClient.get<any>('/whatsapp/groups', { params: { channelId } });
      return data.data || [];
    },
    enabled: !!channelId,
  });
  ...
  if (!selected || selected.data.kind === 'start') {
    return (
      <div className="h-full flex flex-col justify-center px-6 py-8 overflow-y-auto bg-gradient-to-b from-white to-gray-50/50">
        <div className="flex flex-col items-center text-center mb-8">
          <div className="relative w-16 h-16 rounded-2xl bg-gradient-to-br from-blue-500/10 to-indigo-500/10 flex items-center justify-center mb-4 border border-blue-100/50 shadow-sm animate-pulse">
            <Settings2
              className="w-7 h-7 text-blue-600 animate-spin-slow"
              style={{ animationDuration: '8s' }}
            />
            <div className="absolute -bottom-1 -right-1 w-5 h-5 rounded-full bg-indigo-600 text-white flex items-center justify-center text-[10px] font-bold border-2 border-white shadow-sm">
              ℹ️
            </div>
          </div>
          <h3 className="font-black text-sm text-gray-900 tracking-wide">Workspace Inspector</h3>
          <p className="text-[11px] text-gray-400 mt-1.5 max-w-[220px] leading-relaxed font-medium">
            Select any node on the interactive canvas to configure its specific operational logic.
          </p>
        </div>

        <div className="space-y-3 pt-6 border-t border-gray-100">
          <div className="text-[9px] font-black uppercase tracking-widest text-gray-400/80 mb-2">
          ...
          <div className="flex gap-3 p-3 rounded-xl border border-gray-100 bg-[var(--bg-card)] shadow-sm hover:shadow transition-all duration-300">
            ...
            <div>
              <h4 className="text-[10px] font-bold text-gray-800">Add Messages</h4>
              <p className="text-[9px] text-gray-400 mt-0.5 leading-normal font-medium">
                Click buttons in the left panel to inject text replies, catalogs, or media.
              </p>
            </div>
          </div>
          ...
          <div className="flex gap-3 p-3 rounded-xl border border-gray-100 bg-[var(--bg-card)] shadow-sm hover:shadow transition-all duration-300">
            ...
            <div>
              <h4 className="text-[10px] font-bold text-gray-800">Branch Logic</h4>
              <p className="text-[9px] text-gray-400 mt-0.5 leading-normal font-medium">
                Drag connections from condition nodes to route users based on keyword matches.
              </p>
            </div>
          </div>
          ...
          <div className="flex gap-3 p-3 rounded-xl border border-gray-100 bg-[var(--bg-card)] shadow-sm hover:shadow transition-all duration-300">
            ...
            <div>
              <h4 className="text-[10px] font-bold text-gray-800">Pause Workflows</h4>
              <p className="text-[9px] text-gray-400 mt-0.5 leading-normal font-medium">
                Use delays to space out messages perfectly matching natural human conversation
                speeds.
              </p>
            </div>
          </div>
          ...
          <div className="flex gap-3 p-3 rounded-xl border border-gray-100 bg-[var(--bg-card)] shadow-sm hover:shadow transition-all duration-300">
            ...
            <div>
              <h4 className="text-[10px] font-bold text-gray-800">Connect Webhooks</h4>
              <p className="text-[9px] text-gray-400 mt-0.5 leading-normal font-medium">
                Query APIs or sync external CRM data databases in real-time during conversations.
              </p>
            </div>
          </div>
        </div>
  ...
  const addButton = () => {
    onChange({
      buttons: [...(d.buttons || []), { id: uid(), text: 'New Button', action: 'next' as const }],
    });
  };

  const updateButton = (buttonId: string, updates: any) => {
    onChange({
      buttons: ((d['buttons'] || []) as any[]).map((btn) =>
  ...
  const removeButton = (buttonId: string) => {
    onChange({ buttons: ((d['buttons'] || []) as any[]).filter((btn) => btn.id !== buttonId) });
  };

  const addKeyword = () => {
    onChange({ keywords: [...((d['keywords'] || []) as string[]), ''] });
  };

  const updateKeyword = (index: number, value: string) => {
    const updated = [...((d['keywords'] || []) as string[])];
    updated[index] = value;
    onChange({ keywords: updated });
  };

  const removeKeyword = (index: number) => {
    onChange({ keywords: ((d['keywords'] || []) as string[]).filter((_, i) => i !== index) });
  };

  const addListSection = () => {
    const sections = [...((d['listSections'] || []) as ListSection[])];
    sections.push({
  ...
  const removeListSection = (sectionIdx: number) => {
    onChange({
      listSections: ((d['listSections'] || []) as ListSection[]).filter((_, i) => i !== sectionIdx),
    });
  };
  ...
  const addListRow = (sectionIdx: number) => {
    ...
    if (section) {
      sections[sectionIdx] = {
        ...section,
        rows: [...(section.rows || []), { id: uid(), title: '', description: '' }],
      };
    }
    onChange({ listSections: sections });
  };
  ...
  const updateListRow = (
    ...
    if (section && section.rows) {
      ...
      if (rows[rowIdx]) {
        rows[rowIdx] = { ...rows[rowIdx]!, ...updates } as any;
        sections[sectionIdx] = { ...section, rows };
      }
    }
    onChange({ listSections: sections });
  };
      ...
      <div className={`px-4 py-3 ${meta.bgTint} border-b border-gray-200`}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div
              className={`w-8 h-8 rounded-lg ${meta.bgTint} flex items-center justify-center border border-gray-200`}
            >
              ...
              <div className="space-y-3 bg-purple-50/50 dark:bg-purple-500/5 rounded-xl p-4 border border-purple-100">
                <div className="space-y-1.5">
                  ...
                  <Select
                    ...
                    onValueChange={(v) => onChange({ conditionType: v as any })}
                  >
                ...
                <div className="space-y-1.5">
                  ...
                  <Select
                    ...
                    onValueChange={(v) => onChange({ matchType: v as any })}
                  >
                    <SelectTrigger className="h-9 text-sm bg-white">
                      <SelectValue />
                    </SelectTrigger>
            ...
            <>
              ...
              {d.videoPreview && (
                <div className="flex items-center gap-2 bg-blue-50 dark:bg-blue-500/10 border border-blue-100 dark:border-blue-500/20 rounded-xl p-3">
                  ...
                  <button onClick={removeFile('video')} className="text-red-400 hover:text-red-600">
                    <X className="w-4 h-4" />
                  </button>
                </div>
              )}
              {d.audioPreview && (
                <div className="flex items-center gap-2 bg-purple-50 dark:bg-purple-500/10 border border-purple-100 dark:border-purple-500/20 rounded-xl p-3">
                  ...
                  <button onClick={removeFile('audio')} className="text-red-400 hover:text-red-600">
                    <X className="w-4 h-4" />
                  </button>
                </div>
              )}
              {d.documentPreview && (
                <div className="flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-xl p-3">
                  ...
                  <span className="text-xs flex-1 font-medium text-gray-700">
                    Document attached
                  </span>
                  <button
                ...
                {d.buttons?.map((btn) => (
                  <div key={btn.id} className="flex items-center gap-2">
                    ...
                    <Button
                      ...
                      className="h-8 w-8 p-0 text-red-400 rounded-lg"
                    >
                      <X className="w-3.5 h-3.5" />
                    </Button>
                  </div>
              ...
              <div className="space-y-3 bg-amber-50/50 dark:bg-amber-500/5 rounded-xl p-4 border border-amber-100">
                ...
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    className="h-8 text-sm font-mono rounded-lg bg-white"
                  />
              ...
              <div className="space-y-3 bg-slate-50/80 rounded-xl p-4 border border-slate-200">
                <div className="space-y-1.5">
                  ...
                  <div className="text-[10px] text-gray-400">
                    Min 10 seconds. Flow pauses before the next step.
                  </div>
                </div>
              </div>
            </>
            ...
            <>
              ...
              <div className="space-y-3 bg-teal-50/50 rounded-xl p-4 border border-teal-100">
                <div className="space-y-1.5">
                  ...
                  >
                    <SelectTrigger className="h-9 text-sm bg-[var(--bg-card)] rounded-lg">
                ...
                <div className="space-y-2 bg-red-50/50 dark:bg-red-500/5 rounded-xl p-4 border border-red-100">
                  ...
                  <Input
                    ...
                    onChange={async (e) => {
                      ...
                      formData.append('templateId', (d['templateId'] as string) || '');
                ...
                <div className="space-y-3">
                  ...
                  {bodyVarsArray.map((varText: string) => {
                    ...
                    return (
                      <div
                        key={index}
                        className="space-y-1.5 bg-gray-50 rounded-xl p-3 border border-gray-100"
                      >
                        ...
                        <Select
                          ...
                          onValueChange={(type) =>
                            onChange({
                              variableMapping: {
                                ...((d['variableMapping'] as any) || {}),
                                [index]: { type, value: '' },
                              },
                            })
                          }
                        >
                          <SelectTrigger className="h-8 text-xs bg-[var(--bg-card)] rounded-lg">
                            ...
                            onChange={(e) =>
                              onChange({
                                variableMapping: {
                                  ...((d['variableMapping'] as any) || {}),
                                  [index]: {
                                    ...((d['variableMapping'] as any)?.[index] || {}),
                        ...
                        {sampleValue && (
                          <p className="text-[10px] text-gray-400">
                            Sample: <span className="font-semibold">{sampleValue}</span>
                          </p>
                        )}
                      </div>
                    );
            ...
            <>
              ...
              {(d.webhookMethod === 'POST' || d.webhookMethod === 'PUT' || !d.webhookMethod) && (
                <>
                  ...
                  <div className="space-y-2">
                    ...
                    />
                    <div className="text-[10px] text-gray-400">
                      Leave empty to send full contact & conversation data automatically. Or use
                      variables below to build a custom body.
                    </div>
                  </div>
                </>
              )}
              ...
              {d.webhookMethod === 'GET' && (
                <div className="text-[10px] text-gray-400 bg-orange-50/50 rounded-lg p-3 border border-orange-100">
                  GET requests automatically append contact name, phone, email, message,
                  conversation ID, and channel info as query parameters. You can also add variables
                  to the URL using the buttons below.
                </div>
              )}
              ...
              <div className="space-y-2">
                    ...
                    <button
                      ...
                      onClick={() => {
                        if (d.webhookMethod === 'GET') {
                          ...
                          onChange({
                            webhookUrl: currentUrl + `${separator}${v.key}={{${v.key}}}`,
                          });
                        } else {
                      ...
                      title={
                        d.webhookMethod === 'GET' ? `Add ${v.key} to URL` : `Insert {{${v.key}}}`
                      }
                    >
                      {v.label}
                    </button>
                ...
                <div className="text-[10px] text-gray-400 mt-1">
                  Flow variables set by "Set Variable" nodes are also available using{' '}
                  {'{{your_variable_name}}'} syntax.
                </div>
              </div>
            </>
          )}
              ...
              <div className="space-y-3 bg-red-50/50 dark:bg-red-500/5 rounded-xl p-4 border border-red-100">
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    placeholder="e.g., Conversation ended"
                    className="h-9 text-sm rounded-lg bg-white"
                  />
                  <div className="text-[10px] text-gray-400">
                    Displayed on the End node in the canvas
                  </div>
                </div>
              </div>
            </>
                ...
                <div className="space-y-1.5">
                  ...
                  {(contactGroups as any[]).length === 0 && (
                    <div className="text-[10px] text-gray-400 italic">
                      No groups found. Create groups in Contact Management first.
                    </div>
                  )}
                </div>
              </div>
            ...
            <>
              ...
              <div className="space-y-3 bg-cyan-50/50 rounded-xl p-4 border border-cyan-100">
                <div className="space-y-1.5">
                  ...
                  <Select
                    value={d.contactField || 'name'}
                    onValueChange={(v) => onChange({ contactField: v })}
                  >
                    <SelectTrigger className="h-9 text-sm bg-[var(--bg-card)] rounded-lg">
                      <SelectValue />
                    </SelectTrigger>
                ...
                <div className="space-y-1.5">
                  ...
                  />
                  <div className="text-[10px] text-gray-400">
                    Use {'{{variable_name}}'} to insert flow variable values
                  </div>
                </div>
              </div>
            </>
            ...
            <>
              ...
              <div className="space-y-3 bg-violet-50/50 rounded-xl p-4 border border-violet-100">
                ...
                <div className="space-y-1.5">
                  ...
                  <Select
                    value={d.variableSource || 'static'}
                    onValueChange={(v) => onChange({ variableSource: v as any })}
                  >
                    <SelectTrigger className="h-9 text-sm bg-[var(--bg-card)] rounded-lg">
                      <SelectValue />
                    </SelectTrigger>
                ...
                {d.variableSource === 'from_webhook' && (
                  <div className="space-y-1.5">
                    ...
                    />
                    <div className="text-[10px] text-gray-400">
                      Dot-notation path from webhook response JSON
                    </div>
                  </div>
                )}
              </div>
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <Button
                        ...
                        className="h-7 w-7 p-0 text-red-400 rounded-lg"
                      >
                        <X className="w-3 h-3" />
                      </Button>
                    </div>
                    ...
                    <Button
                      ...
                      className="h-6 text-[10px] text-sky-600 hover:text-sky-700 hover:bg-sky-50 w-full rounded-lg"
                    >
                  ...
                  <Select
                    ...
                    onValueChange={(v) =>
                      onChange({
                        ...
                        mediaId: '',
                        mediaFileName: '',
                        mediaSourceType: d.mediaSourceType || 'url',
                      })
                    }
                  >
                    <SelectTrigger className="h-9 text-sm bg-[var(--bg-card)] rounded-lg">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex gap-1 p-0.5 bg-gray-100 rounded-lg">
                    <button
                      ...
                      onClick={() =>
                        onChange({ mediaSourceType: 'url', mediaId: '', mediaFileName: '' })
                      }
                    >
                        ...
                        <input
                          ...
                          onChange={async (e) => {
                            ...
                            try {
                              ...
                              formData.append('mediaFile', file);
                              const res = await fetch(
                                `/api/whatsapp/channels/${channelId}/upload-media`,
                                {
                                  method: 'POST',
                                  body: formData,
                                  credentials: 'include',
                          ...
                          {mediaUploading ? (
                            ...
                            <>
                              ...
                              <span className="text-xs text-gray-500">
                                Click to upload {d.mediaType || 'image'}
                              </span>
                            </>
                          )}
                        </label>
                    ...
                    <div className="text-[10px] text-gray-400">
                      {d.mediaType === 'image'
                        ...
                        : d.mediaType === 'video'
                          ? 'MP4, 3GPP (max 16MB)'
                          : d.mediaType === 'audio'
                            ? 'AAC, MP4, MPEG, OGG, Opus (max 16MB)'
              ...
              <div className="space-y-2 bg-lime-50/50 rounded-xl p-4 border border-lime-100">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-gray-600">
                    Sends read receipts (blue ticks) for the last incoming message from the
                    customer.
                  </span>
                </div>
                <div className="text-[10px] text-gray-400">
                  No configuration needed. This node automatically marks the conversation as read.
                </div>
              </div>
            </>
          )}
