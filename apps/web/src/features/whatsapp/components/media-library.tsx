      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {isLoading ? (
          Array.from({ length: pageSize }).map((_, i) => (
            <div
              key={i}
              className="appt-card animate-pulse bg-pp-bg-subtle/20 border-pp-border h-[200px]"
            />
          ))
        ) : media.length === 0 ? (
          <div className="col-span-full py-20 text-center">
            ...
            <p className="text-secondary text-sm mt-1">
              Upload educational imagery or medical documents to reuse them in campaigns.
            </p>
          </div>
        ) : (
          media.map((item: any) => (
            <div
              key={item.id}
              className="appt-card group hover:border-pp-blue/30 transition-all p-0 overflow-hidden flex flex-col"
            >
              <div className="aspect-video bg-pp-bg-subtle flex items-center justify-center relative">
                {item.type === 'image' && item.url ? (
                  <img src={item.url} alt={item.name} className="w-full h-full object-cover" />
                ) : (
                  <div className="text-primary/30">{getFileIcon(item.type)}</div>
                )}
