...
export const MediaModal = ({ isOpen, onClose }: MediaModalProps) => {
  ...
  const onSubmit = (data: MediaFormInput) => {
    if (!data.file || data.file.length === 0) {
      toast({
        title: 'File Required',
        description: 'Please select a medical asset to vault.',
        variant: 'error',
      });
      return;
    }
    ...
    if (!channelId) {
      toast({
        title: 'No Channel',
        description: 'Please connect a Meta Business channel first.',
        variant: 'error',
      });
      return;
    }
    ...
    uploadMediaMutation.mutate(
      { channelId, file: data.file[0] as unknown as File, title: data.name },
      {
        onSuccess: () => {
          toast({
            title: 'Asset Vaulted',
            description: 'Medical media has been securely uploaded to Meta.',
          });
          reset();
          onClose();
        },
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="appt-form">
            ...
            <div className="appt-form-group">
              ...
              <p className="text-[10px] text-muted mt-1 px-1">
                Upload securely to Meta Cloud via End-to-End Encrypted pipeline.
              </p>
            </div>
            ...
            <div className="p-4 bg-amber-500/5 border border-amber-500/10 rounded-xl flex gap-3 mt-6">
              ...
              <div className="space-y-1">
                ...
                <p className="text-[11px] text-secondary leading-relaxed">
                  Maximum file size is 16MB for images and 64MB for clinical video educational
                  content.
                </p>
              </div>
            </div>
            ...
            <div className="appt-form-actions" style={{ marginTop: 'auto', paddingTop: '24px' }}>
              ...
              >
                {uploadMediaMutation.isPending ? (
                  <>
                    <Loader2 size={15} className="animate-spin" /> Vaulting…
                  </>
                ) : (
                  'Vault Asset'
                )}
