    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 animate-fade-in">
      <div className="bg-white rounded-2xl w-full max-w-md p-6 shadow-2xl border border-[var(--pp-warm-3)] animate-scale-in">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-sm font-extrabold text-[var(--pp-ink)] uppercase tracking-[0.1em]">
            Forward Message
          </h3>
          <button
            onClick={onClose}
            className="p-1 hover:bg-[var(--pp-warm-2)] rounded-lg text-[var(--pp-text-3)] hover:text-[var(--pp-ink)] transition-all"
          >
            <X size={18} />
          </button>
        </div>
        <div className="relative mb-4">
          <Search
            className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--pp-text-3)]/60"
            size={14}
          />
          <input
        ...
        <div className="max-h-60 overflow-y-auto divide-y divide-[var(--pp-warm-3)]/40 pr-1">
          {filtered.length === 0 ? (
            <div className="p-4 text-center text-xs text-[var(--pp-text-3)] italic">
              No contacts found
            </div>
          ) : (
            filtered.map((conv) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <h4 className="text-xs font-bold text-[var(--pp-ink)] truncate">
                    {conv.contactName || `+${conv.contactPhone}`}
                  </h4>
                  <p className="text-[10px] text-[var(--pp-text-3)] truncate">
                    +{conv.contactPhone}
                  </p>
                </div>
              </div>
...
export const Inbox = ({ channelId }: { channelId?: number }) => {
  ...
  const {
    ...
    error: msgError,
  } = useMessages(selectedConvId || undefined);
  ...
  React.useEffect(() => {
    ...
    if (phoneParam && !loadingConv) {
      ...
      const existing = conversations?.find(
        (c) => c.contactPhone === targetPhone || c.contactPhone === `91${targetPhone}`,
      );
  ...
  const handleUpdateStatus = (newStatus: 'open' | 'closed') => {
    ...
    updateStatusMutation.mutate(
      { id: selectedConvId, status: newStatus },
      {
        onSuccess: () => {
          toast({
  ...
  const handleViewContact = async () => {
    ...
    try {
      ...
      const { data: lookupRes } = await apiClient.get<{ success: boolean; data: any[] }>(
        '/patients/lookup',
        {
  ...
  const handleArchiveChat = () => {
    ...
    const newTags = Array.isArray(existingTags)
      ? existingTags.includes('archived')
        ? existingTags
        : [...existingTags, 'archived']
      : ['archived'];
    ...
    sendMessageMutation.mutate(payload, {
      ...
      onError: (err: any) => {
        ...
        if (
          ...
          errorMsg.includes('conversation window') ||
          errorMsg.includes('window')
        ) {
          toast({
            title: 'Meta Security Restriction',
            description:
              'This contact hasn\'t messaged you in 24 hours. Please click the yellow "Send Template" banner above the input field to start the conversation.',
            variant: 'error',
          });
        } else {
          toast({
      ...
      },
      {
        onSuccess: () => {
          toast({
            ...
            description: `Message forwarded to ${destConv.contactName || destConv.contactPhone}.`,
          });
  ...
  const handleStartChat = (contact: any) => {
    ...
    if (existing) {
      ...
      toast({
        ...
        description: `Initiating secure dialogue with ${contact.name}...`,
      });
      createConversationMutation.mutate(
        {
  ...
  const filteredConversations = conversations
    ?.filter((conv) => {
      ...
      if (activeStatusFilter === 'open' && (conv.status === 'resolved' || conv.status === 'closed'))
        return false;
      if (
  ...
  return (
    <div className="flex w-full max-w-full h-[calc(100vh-220px)] border border-[var(--pp-warm-4)] rounded-2xl overflow-hidden bg-[var(--bg-card)] shadow-[0_4px_24px_rgba(0,0,0,0.03)] animate-fade-in">
      <div
        className={`w-full md:w-80 lg:w-96 min-w-0 border-r border-[var(--pp-warm-3)] flex flex-col bg-[var(--pp-warm-1)]/40 backdrop-blur-md ${selectedConvId ? 'hidden md:flex' : 'flex'}`}
      >
        <div className="p-5 border-b border-[var(--pp-warm-3)] space-y-4 bg-white">
          <div className="flex justify-between items-center">
            <h3 className="text-[11px] font-extrabold text-[var(--pp-text-3)] uppercase tracking-[0.12em]">
              Clinical Team Chat
            </h3>
            <button
          ...
          <div className="relative group">
            <Search
              className="absolute left-4 top-1/2 -translate-y-1/2 text-[var(--pp-text-3)]/50 transition-colors group-focus-within:text-[var(--pp-blue)]"
              size={14}
            />
            <input
        ...
        <div className="flex-1 overflow-y-auto divide-y divide-[var(--pp-warm-3)]/40">
          {loadingConv ? (
            <div className="p-8 text-center text-[var(--pp-text-3)] text-xs font-bold animate-pulse uppercase tracking-widest">
              Loading chats...
            </div>
          ) : filteredConversations?.length === 0 ? (
            <div className="p-12 text-center text-[var(--pp-text-3)] italic text-sm">
              No clinical chats found.
            </div>
          ) : (
                ...
                <div className="flex gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-start mb-0.5">
                      <h4
                        className={`font-bold text-[13px] truncate flex items-center gap-1.5 ${selectedConvId === conv.id ? 'text-[var(--pp-blue)]' : 'text-[var(--pp-ink)]'}`}
                      >
                        {Array.isArray(conv.tags) && conv.tags.includes('pinned') && (
                          <Pin
                            size={11}
                            fill="currentColor"
                            className="rotate-45 shrink-0 text-[var(--pp-blue)]"
                          />
                        )}
      ...
      {/* Main Content - Chat */}
      <div
        className={`flex-1 min-w-0 flex flex-col bg-[var(--bg-card)] ${selectedConvId ? 'flex' : 'hidden md:flex'}`}
      >
        {selectedConvId ? (
          <>
            <div className="px-6 py-4 border-b border-[var(--pp-warm-3)] flex justify-between items-center bg-[var(--bg-card)] shadow-sm z-10">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-[var(--pp-ink)] leading-tight text-sm">
                  ...
                  <div className="flex items-center gap-1.5 mt-1">
                    <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-[var(--pp-success-bg)] border border-[var(--pp-success-border)]/50">
                      ...
                      <span className="text-[9px] font-extrabold text-[var(--pp-success-fg)] uppercase tracking-wider">
                        Patient Active
                      </span>
                    </div>
                  </div>
              ...
              <div className="flex items-center gap-1">
                <button
                  ...
                  title={
                    (selectedConv as any)?.tags?.includes('pinned') ? 'Unpin Chat' : 'Pin Chat'
                  }
                >
                  <Pin
                    size={15}
                    fill={(selectedConv as any)?.tags?.includes('pinned') ? 'currentColor' : 'none'}
                    className={(selectedConv as any)?.tags?.includes('pinned') ? 'rotate-45' : ''}
                  />
                </button>
                  ...
                  {showMenu && (
                    <>
                      ...
                      <div className="absolute right-0 mt-2 w-56 bg-[var(--bg-card)] border border-[var(--pp-warm-3)]/80 rounded-2xl shadow-2xl py-2 z-50 animate-scale-in text-left origin-top-right">
                        <div className="px-4 py-1.5 text-[10px] font-extrabold text-[var(--pp-text-3)] uppercase tracking-[0.12em]">
                          Status
                        </div>
                        ...
                        {Array.isArray((selectedConv as any)?.tags) &&
                        (selectedConv as any).tags.includes('archived') ? (
                          <button
                        ...
                        {Array.isArray((selectedConv as any)?.tags) &&
                        (selectedConv as any).tags.includes('blocked') ? (
                          <button
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-gradient-to-b from-[var(--pp-warm-1)]/50 to-[var(--pp-warm-2)]/30">
              {loadingMsg ? (
                <div className="flex justify-center p-8 text-xs font-bold text-[var(--pp-text-3)] uppercase tracking-widest animate-pulse">
                  Syncing Dialogue...
                </div>
              ) : (
                messages
                  ...
                  .map((msg: any) => (
                    ...
                    >
                      <div
                        className={`flex items-center gap-2 max-w-[75%] ${msg.direction === 'outbound' ? 'flex-row-reverse' : 'flex-row'}`}
                      >
                        <div
                        ...
                        >
                          {msg.metadata?.replyTo && (
                            <div
                              className={`mb-2 p-2 rounded-lg text-xs border-l-4 leading-normal flex flex-col gap-0.5 ${
                                msg.direction === 'outbound'
                            ...
                            >
                              ...
                              <span className="truncate italic font-medium">
                                {msg.metadata.replyTo.content}
                              </span>
                            </div>
                          )}
                          <p className="text-[13px] leading-relaxed font-semibold whitespace-pre-wrap">
                            {msg.content}
                          </p>
                          <div
                            className={`flex items-center gap-1.5 mt-2 ${msg.direction === 'outbound' ? 'justify-end' : 'justify-start'}`}
                          >
                            <span
                              className={`text-[8px] font-extrabold uppercase tracking-widest ${msg.direction === 'outbound' ? 'text-white/60' : 'text-[var(--pp-text-3)]'}`}
                            >
            ...
            {pendingAttachment && (
              <div className="px-6 py-2.5 bg-[var(--pp-warm-1)] border-t border-[var(--pp-warm-3)] flex items-center justify-between animate-fade-in z-20">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-xs font-bold text-[var(--pp-ink)]">
                      {pendingAttachment.fileName}
                    </p>
                    <p className="text-[10px] text-[var(--pp-text-3)] font-extrabold uppercase tracking-wider">
            ...
            {!isWindowActive && (
              <div className="px-6 py-3 bg-amber-50 dark:bg-amber-500/10 border-t border-b border-amber-200/60 flex items-center justify-between text-amber-800 text-xs font-semibold animate-fade-in z-20 gap-4">
                <div className="flex items-center gap-2">
                  ...
                  <span>
                    Meta Rule: The 24-hour message window is currently closed. You must send an
            ...
            <div className="p-4 border-t border-[var(--pp-warm-3)] bg-[var(--bg-card)] flex items-center gap-3">
              ...
              <button
                onClick={handleSend}
                disabled={
                  sendMessageMutation.isPending || (!messageText.trim() && !pendingAttachment)
                }
          ...
          <div className="flex-1 flex flex-col items-center justify-center bg-[var(--pp-warm-1)]/30 p-12 text-center">
            ...
            <h3 className="text-sm font-extrabold text-[var(--pp-ink)] uppercase tracking-[0.15em]">
          ...
          sendTemplateMutation.mutate(
            ...
            },
            {
              onSuccess: () => {
                toast({
                  title: 'Template Sent',
                  description: `${template.name} delivered to patient.`,
                });
              },
              onError: (err: any) => {
                toast({
