...
const ChannelList = lazy(() =>
  import('../components/channel-list').then((m) => ({ default: m.ChannelList })),
);
const Inbox = lazy(() => import('../components/inbox').then((m) => ({ default: m.Inbox })));
const ContactList = lazy(() =>
  import('../components/contact-list').then((m) => ({ default: m.ContactList })),
);
const AutomationBuilder = lazy(() =>
  import('../components/automation-builder').then((m) => ({ default: m.AutomationBuilder })),
);
const ChatbotManager = lazy(() =>
  import('../components/chatbot-manager').then((m) => ({ default: m.ChatbotManager })),
);
const MediaLibrary = lazy(() =>
  import('../components/media-library').then((m) => ({ default: m.MediaLibrary })),
);
const Templates = lazy(() =>
  import('../components/templates').then((m) => ({ default: m.Templates })),
);
const Analytics = lazy(() =>
  import('../components/analytics').then((m) => ({ default: m.Analytics })),
);
const WidgetBuilder = lazy(() =>
  import('../components/widget-builder').then((m) => ({ default: m.WidgetBuilder })),
);
      ...
      case 'automations':
        return {
          ...
          actions: (
            <button
              className="btn-primary h-10 px-4"
              onClick={() => window.dispatchEvent(new CustomEvent('open-automation-modal'))}
            >
              <Zap size={15} />
              Create Workflow
            </button>
            ...
            <button
              ...
              onClick={() =>
                toast({
                  ...
                  description:
                    'Your AI agent is already bound to this clinic channel. Please configure its knowledge base below.',
                })
              }
            >
              ...
              <div className="appt-card p-6 flex flex-col justify-between group hover:border-pp-blue transition-all duration-300">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <h3
                      className={`text-3xl font-bold transition-all ${loadingAnalytics ? 'animate-pulse text-muted/30' : 'text-main'}`}
                    >
                      {loadingAnalytics
                        ? '...'
                        : ((analytics as WhatsAppAnalytics)?.totalDeliveries || 0).toLocaleString()}
                    </h3>
                  </div>
              ...
              <div className="appt-card p-6 flex flex-col justify-between group hover:border-success transition-all duration-300">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <h3 className="text-3xl font-bold text-main">98.6%</h3>
                  </div>
              ...
              <div className="appt-card p-6 flex flex-col justify-between group hover:border-purple-500 transition-all duration-300">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <h3 className="text-3xl font-bold text-main">84.2%</h3>
                  </div>
                ...
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    >
                      {loadingAnalytics
                        ? '...'
                        : ((analytics as WhatsAppAnalytics)?.campaignReach || 0).toLocaleString()}
                    </h3>
                  </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="appt-card p-6 lg:col-span-2 flex flex-col justify-between">
                <div className="flex justify-between items-center mb-6">
                  <div>
                    ...
                    <p className="text-xs text-secondary mt-0.5">
                      Real-time daily telemetry and dispatch activity.
                    </p>
                  </div>
                ...
                <div className="h-64 w-full">
                  {loadingAnalytics ? (
                    <div className="h-full flex items-center justify-center text-xs font-bold uppercase tracking-wider text-muted/40 animate-pulse">
                      Aggregating telemetric trends...
                    </div>
                  ) : (analytics as WhatsAppAnalytics)?.trendData?.length ? (
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart
                        data={(analytics as WhatsAppAnalytics).trendData}
                    ...
                    <div className="h-full flex items-center justify-center text-xs font-bold uppercase tracking-wider text-muted/40 border border-dashed rounded-2xl bg-pp-bg-subtle/40">
                      No analytics data reported.
                    </div>
                  )}
                </div>
              </div>
                ...
                <div>
                  ...
                  {activeChannel ? (
                    <div className="space-y-4">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-[11px] font-semibold text-secondary mt-0.5">
                              +{activeChannel.phoneNumber.slice(0, 3)}******
                              {activeChannel.phoneNumber.slice(-4)}
                            </p>
                          </div>
                        </div>
                        <span className="pp-badge-status status-active text-[9px] font-bold">
                          CONNECTED
                        </span>
                      </div>
                      ...
                      <div className="p-4 bg-amber-50/30 dark:bg-amber-500/5 border border-amber-100 dark:border-amber-500/20 rounded-2xl">
                        <div className="flex justify-between text-xs font-bold text-main mb-1.5">
                          ...
                          <span>
                            {(analytics as WhatsAppAnalytics)?.activeConversations || 0} / 1,000
                          </span>
                        </div>
                        <div className="w-full bg-slate-100 rounded-full h-2 overflow-hidden">
                          <div
                            ...
                            style={{
                              width: `${Math.min((((analytics as WhatsAppAnalytics)?.activeConversations || 0) / 1000) * 100, 100)}%`,
                            }}
                          />
                        </div>
                      </div>
                    ...
                    <div className="text-center py-12 bg-[var(--bg-main)] border border-dashed rounded-2xl">
                      ...
                      <p className="text-xs text-secondary max-w-[200px] mx-auto mt-1 mb-4">
                        Integrate WABA credentials to unlock telemetry.
                      </p>
                      <button
                        ...
                        onClick={() => {
                          ...
                          setTimeout(
                            () => window.dispatchEvent(new CustomEvent('open-channel-modal')),
                            150,
                          );
                        }}
                      >
                ...
                <div className="pt-4 border-t border-pp-border flex justify-between items-center">
                  ...
                  {activeChannel && (
                      ...
                      onClick={() => {
                        syncTemplatesMutation.mutate(activeChannel.id, {
                          onSuccess: () =>
                            toast({
                              title: 'Success',
                              description: 'Meta Templates synchronized successfully.',
                            }),
                          onError: () =>
                            toast({
                              title: 'Error',
                    ...
                    >
                      <RefreshCcw
                        size={10}
                        className={syncTemplatesMutation.isPending ? 'animate-spin' : ''}
                      />
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                >
                  ...
                  <p className="text-[10px] text-secondary mt-0.5">
                    Send custom updates and alerts.
                  </p>
                </button>
                ...
                >
                  ...
                  <p className="text-[10px] text-secondary mt-0.5">
                    Configure keyword responder flows.
                  </p>
                </button>
                ...
                >
                  ...
                  <p className="text-[10px] text-secondary mt-0.5">
                    Customize floating chat triggers.
                  </p>
                </button>
              </div>
            </div>
  ...
  return (
    <div className="pp-page-container animate-fade-in">
      ...
      <div className="pp-page-hero">
        ...
        <div className="pp-page-hero-actions">{config.actions}</div>
      </div>
