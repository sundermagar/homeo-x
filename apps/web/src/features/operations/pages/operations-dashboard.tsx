...
export default function OperationsDashboard() {
  ...
  const [modalType, setModalType] = useState<
    'courier' | 'lead' | 'dictionary' | 'export' | 'referral' | 'reminder' | 'book' | null
  >(null);
  const [viewMode, setViewMode] = useState<'list' | 'grid'>('list');

  // ─── CRM Tab Sub-Navigation ───
  const [crmSection, setCrmSection] = useState<'pipeline' | 'referrals' | 'reminders'>('pipeline');
  const [crmFilter, setCrmFilter] = useState<'all' | 'new' | 'pending' | 'converted' | 'overdue'>(
    'all',
  );
  ...
  const headers: Record<GenericTab, { title: string; desc: string }> = {
    logistics: {
      ...
      desc: 'Manage shipping vendors and track active medicine deliveries.',
    },
    crm: {
      title: 'Lead CRM & Promos',
      desc: 'Capture new patient leads, track network referrals, and schedule reminders.',
    },
    knowledge: {
      title: 'Medical Knowledge Base',
      desc: 'Access global diagnosis terminology and uploaded reference books.',
    },
    tools: {
      title: 'Global Data Tools',
      desc: 'Administer database exports and system-wide backups.',
    },
  };

  const actionMap: Record<
    GenericTab,
    { label: string; type: 'courier' | 'lead' | 'dictionary' | 'export' | null }
  > = {
  ...
  return (
    <div className="operations-dashboard fade-in">
        ...
        actions={
          activeAction.type ? (
            <button
              ...
              onClick={() => setModalType(activeAction.type)}
            >
              {activeAction.label}
            </button>
          ) : null
      ...
      {activeTab === 'logistics' && (
        <div className="slide-up">
          <div className="ops-stats-row">
            ...
            <StatCard
              icon={CheckCircle2}
              value={shipments.filter((s) => (s.status || '').toLowerCase() === 'delivered').length}
              label="Delivered"
              variant="success"
            />
            <StatCard
              icon={AlertCircle}
              value={shipments.filter((s) => (s.status || '').toLowerCase() === 'pending').length}
              label="Pending"
              variant="danger"
            />
          </div>
          ...
          <div className="ops-content card">
            <div className="ops-table-header" style={{ alignItems: 'flex-start' }}>
              <div>
                ...
                <p style={{ fontSize: '0.72rem', color: 'var(--pp-muted)', marginTop: 4 }}>
                  Real-time tracking of medicine dispatches.
                </p>
              </div>
                    ...
                    {shipments
                      ...
                      .map((s) => (
                        <tr key={s.id} className="plat-table-row">
                          <td>
                            <span className="reg-badge">#{s.regid || '-'}</span>
                          </td>
                          <td>
                            <div className="cell-main">
                              {s.patient_name || s.patient || `Patient #${s.regid || '-'}`}
                            </div>
                    ...
                    <div key={s.id} className="ops-card" style={{ padding: 16, borderRadius: 18 }}>
                      ...
                      >
                        <div>
                          ...
                          <div className="cell-main" style={{ fontSize: '15px' }}>
                            {s.patient_name || s.patient || `Patient #${s.regid || '-'}`}
                          </div>
                          <div className="cell-sub">
                            <Phone size={11} /> {s.mobile || '-'}
                      ...
                      >
                        ...
                        <button
                          ...
                          style={{ padding: '6px 12px', fontSize: '11px', borderRadius: 8 }}
                        >
                          Details
                        </button>
                      </div>
      ...
      {activeTab === 'crm' && (
        <div className="slide-up">
          <div className="ops-stats-row">
            ...
            <StatCard
              icon={AlertCircle}
              value={leads.filter((l) => !l.status || l.status.toLowerCase() === 'new').length}
              label="New"
              variant="info"
            />
            <StatCard
              icon={CheckCircle2}
              value={leads.filter((l) => l.status?.toLowerCase() === 'converted').length}
              label="Converted"
              variant="success"
            />
          ...
          {crmSection === 'pipeline' && (
            <div className="ops-content card slide-up">
              <>
                ...
                <div
                  className="crm-filter-pills"
                  style={{ marginBottom: 20, borderBottom: 'none' }}
                >
                      ...
                      <tbody>
                        {leads
                          .filter((l) => {
                            ...
                            const matchesFilter =
                              crmFilter === 'all' || l.status?.toLowerCase() === crmFilter;
                          ...
                          .map((l) => (
                            <tr key={l.id} className="plat-table-row">
                              ...
                              <td>
                                ...
                                <div className="cell-sub">
                                  <Phone size={11} /> {l.mobile || l.phone}
                                </div>
                              </td>
                              <td>{l.source}</td>
                              <td>
                              ...
                              <td>
                                <div
                                  className="cell-sub"
                                  style={{ maxWidth: 500, lineHeight: 1.4 }}
                                >
                                  {l.notes}
                                </div>
                              </td>
              ...
              <>
                <div className="ops-table-header">
                  <div>
                    ...
                    <p style={{ fontSize: '0.72rem', color: 'var(--pp-muted)', marginTop: 4 }}>
                      Track patient referral network and incentives.
                    </p>
                  </div>
                  <button
                    className="ops-btn ops-btn-primary"
                    onClick={() => setModalType('referral')}
                  >
                        ...
                        {referrals
                          ...
                          .map((r, idx) => (
                            <tr key={idx} className="plat-table-row">
                              ...
                              <td>
                                <span className="cell-sub">ID: {r.referral_id}</span>
                              </td>
                              <td>
                                <span style={{ color: 'var(--pp-success-fg)', fontWeight: 600 }}>
                                  ₹{r.total_amount}
                                </span>
                              </td>
                              <td>₹{r.used_amount}</td>
              ...
              <>
                <div className="ops-table-header">
                  <div>
                    ...
                    <p style={{ fontSize: '0.72rem', color: 'var(--pp-muted)', marginTop: 4 }}>
                      Upcoming patient follow-ups and case reviews.
                    </p>
                  </div>
                  <button
                    className="ops-btn ops-btn-primary"
          ...
          <div className="ops-content card" style={{ marginBottom: 16 }}>
            <div className="ops-table-header" style={{ alignItems: 'flex-start' }}>
              <div>
                ...
                <p style={{ fontSize: '0.72rem', color: 'var(--pp-muted)', marginTop: 4 }}>
                  Diagnosis terminology and clinical references.
                </p>
              </div>
              <div className="appt-segmented-toggle">
                <button
                    ...
                    {dictionary
                      ...
                      .map((d) => (
                        <tr key={d.id} className="plat-table-row">
                          <td>
                            <span className="cell-main" style={{ fontWeight: 700 }}>
                              {d.title}
                            </span>
                          </td>
                          <td>
                            <span className="cell-sub">{d.text}</span>
                          </td>
                          <td>
                            <span className="ops-cross-ref">{d.cross_ref || d.crossRef}</span>
                          </td>
                        </tr>
                      ))}
                  </tbody>
          ...
          <div className="ops-content card">
            <div className="ops-table-header" style={{ alignItems: 'flex-start' }}>
              <div>
                ...
                <p style={{ fontSize: '0.72rem', color: 'var(--pp-muted)', marginTop: 4 }}>
                  Uploaded clinical books and PDFs.
                </p>
              </div>
                  ...
                  .map((b) => (
                    ...
                    >
                      ...
                      >
                        ...
                        <button
                          className="ops-btn ops-btn-ghost"
                          style={{ padding: '6px 10px', fontSize: '11px', borderRadius: 8 }}
                        >
                          Open
                        </button>
                      </div>
          ...
          <div className="ops-content card">
            ...
            <div className="ops-table-wrapper">
              <table className="ops-table">
                <thead>
                  <tr>
                    {['Dataset', 'Description', 'Action'].map((col) => (
                      <th key={col}>{col}</th>
                    ))}
                  </tr>
                </thead>
                  ...
                  {exports.map((e) => (
                    <tr key={e.id}>
                      <td data-label="Dataset">
                        <span className="cell-main">{e.label}</span>
                      </td>
                      <td data-label="Description">
                        <span className="cell-sub">Click to download CSV</span>
                      </td>
      ...
      <div className={`ops-drawer-overlay ${modalType ? 'active' : ''}`} onClick={closeModal}>
        <div
          className={`ops-drawer ${modalType ? 'active' : ''}`}
        ...
        >
          <div className="ops-drawer-header">
            ...
            <button className="ops-drawer-close" onClick={closeModal}>
          ...
          <div className="ops-drawer-body">
              ...
              <>
                ...
                <div className="ops-form-group">
                  ...
                  <NumericInput
                    ...
                    onValueChange={(val: string) => setFormData({ ...formData, mobile: val })}
                  />
                </div>
                <div className="ops-form-group">
              ...
              <>
                ...
                <div className="ops-form-group">
                  ...
                  <textarea
                    ...
                    style={{ minHeight: 120 }}
                    value={dictForm.text}
                    onChange={(e) => setDictForm({ ...dictForm, text: e.target.value })}
                  />
                </div>
                <div className="ops-form-group">
                  ...
                  <input
                    type="text"
                    className="ops-input"
                    value={dictForm.cross_ref}
                    onChange={(e) => setDictForm({ ...dictForm, cross_ref: e.target.value })}
                  />
                </div>
              </>
            )}
            ...
            {modalType === 'book' && (
              <>
                ...
                <div className="ops-form-group">
                  ...
                  <select
                    className="ops-input"
                    value={bookForm.resource_type}
                    onChange={(e) => setBookForm({ ...bookForm, resource_type: e.target.value })}
                  >
            ...
            {modalType === 'export' && (
              <div className="ops-form-group">
                <label>Select Dataset</label>
                <select className="ops-input">
                  <option>All Patients via CSV</option>
                  <option>Medicines & Inventory (Excel)</option>
                  <option>Full Backup Dump (.sql)</option>
                </select>
              </div>
            )}
          </div>
          ...
          <div className="ops-drawer-footer">
            <button className="ops-btn ops-btn-ghost" onClick={closeModal}>
              Cancel
            </button>
