...
export function PatientBillingDrawer({ regid, patientName, isOpen, onClose }: Props) {
  ...
  const renderPastGroupCard = (group: any) => {
    ...
    return (
      <div key={group.billDate} style={{ padding: '16px', background: 'var(--bg-card)', borderRadius: 16, border: '1px solid var(--border-main)', display: 'flex', flexDirection: 'column', gap: 12 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            <div style={{ fontWeight: 800, fontSize: '1.05rem', color: 'var(--pp-ink)' }}>{format(new Date(group.billDate), 'dd MMM yyyy')}</div>
            <div style={{ fontSize: '0.75rem', color: 'var(--pp-text-3)', fontWeight: 600, marginTop: 2 }}>{group.bills.length} Invoice Item{group.bills.length !== 1 ? 's' : ''}</div>
          </div>
          {isPaid ? <span style={{ fontSize: '0.65rem', fontWeight: 800, color: 'var(--pp-success-fg)', background: 'var(--pp-success-bg)', padding: '2px 8px', borderRadius: 12 }}>PAID</span> : <span style={{ fontSize: '0.65rem', fontWeight: 800, color: 'var(--pp-danger-fg)', background: 'var(--pp-danger-bg)', padding: '2px 8px', borderRadius: 12 }}>UNPAID</span>}
        </div>
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 8, paddingTop: 12, borderTop: '1px dashed var(--pp-warm-3)' }}>
          {group.totalBalance > 0 && (
            <button className="bill-btn bill-btn-primary" style={{ flex: 1, height: 32, borderRadius: 8, fontSize: '0.75rem' }} onClick={() => { setReceivingGroup(group); setReceiveAmount(group.totalBalance); }}>
              Receive ₹{group.totalBalance.toLocaleString()}
            </button>
          )}
          <button className="bill-btn" style={{ flex: group.totalBalance > 0 ? 'none' : 1, width: group.totalBalance > 0 ? 'auto' : '100%', height: 32, borderRadius: 8, fontSize: '0.75rem', border: '1px solid var(--pp-warm-3)' }} onClick={() => { if (myOrg) printGroupedBills(group, myOrg); }}>
      ...
      <Drawer isOpen={isOpen} onClose={onClose} title="Patient Billing Details" maxWidth="600px">
        {latestGroup ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <div style={{ padding: '24px 20px', background: 'linear-gradient(135deg, var(--pp-blue-tint) 0%, rgba(37, 99, 235, 0.05) 100%)', borderRadius: 24, border: '1px solid var(--pp-blue-border)', display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
                <div style={{ background: 'var(--bg-card)', border: '1px solid var(--pp-warm-3)', borderRadius: 16, padding: '12px 14px', boxShadow: 'var(--pp-shadow-sm)', display: 'flex', flexDirection: 'column', gap: 4 }}>
                  ...
                  <span style={{ fontSize: '1.2rem', fontWeight: 850, color: 'var(--pp-blue)', fontFamily: 'var(--pp-font-mono)' }}>₹{latestGroup.totalCharges.toLocaleString()}</span>
                </div>
                <div style={{ background: 'var(--bg-card)', border: '1px solid var(--pp-warm-3)', borderRadius: 16, padding: '12px 14px', boxShadow: 'var(--pp-shadow-sm)', display: 'flex', flexDirection: 'column', gap: 4 }}>
                ...
                <div style={{ background: latestGroup.totalBalance > 0 ? 'var(--pp-danger-bg)' : 'var(--bg-card)', border: latestGroup.totalBalance > 0 ? '1px solid var(--pp-danger-border)' : '1px solid var(--pp-warm-3)', borderRadius: 16, padding: '12px 14px', boxShadow: 'var(--pp-shadow-sm)', display: 'flex', flexDirection: 'column', gap: 4 }}>
              ...
              <div style={{ background: 'var(--bg-card)', border: '1px solid var(--pp-warm-3)', borderRadius: 16, padding: 16, boxShadow: 'var(--pp-shadow-sm)', display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {[
                    { label: 'Registration Fee', val: latestGroup.registrationCharge, color: 'var(--pp-blue)' },
                    { label: 'Medicine & Consultation', val: latestGroup.medicineDaysCharge, color: 'var(--pp-warning-fg)' },
              ...
              <div style={{ display: 'flex', gap: 10, borderTop: '1px dashed var(--pp-blue-border)', paddingTop: 16 }}>
                {latestGroup.totalBalance > 0 && (
                  <button className="bill-btn bill-btn-primary" style={{ flex: 1, height: 40, borderRadius: 12, fontSize: '0.82rem', fontWeight: 750, gap: 6 }} onClick={() => { setReceivingGroup(latestGroup); setReceiveAmount(latestGroup.totalBalance); }}>
                    <DollarSign size={14} /> Receive Total (₹{latestGroup.totalBalance.toLocaleString()})
                  </button>
                )}
                <button className="bill-btn" style={{ flex: latestGroup.totalBalance > 0 ? 'none' : 1, width: latestGroup.totalBalance > 0 ? 'auto' : '100%', height: 40, padding: '0 16px', borderRadius: 12, background: 'var(--bg-card)', border: '1px solid var(--border-main)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, fontSize: '0.82rem', fontWeight: 750, color: 'var(--pp-ink)' }} onClick={() => { if (myOrg) printGroupedBills(latestGroup, myOrg); }}>
