...
export function BillingForm({
  ...
  const [selectedPatient, setSelectedPatient] = useState<{ regid: number; name: string } | null>(
    null,
  );
  ...
  return (
    <form
      ...
      className="bill-card bill-form-card"
    ...
    >
              ...
              <div className="bill-card bill-dropdown-overlay">
                {patientsQuery.data.data.map((p: any) => (
                  ...
                  >
                    ...
                    <div className="bill-dropdown-item-meta">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          ...
          <div className="bill-form-group">
            <label className="bill-form-label">
              Charges (₹) <span style={{ color: 'var(--pp-danger-fg)' }}>*</span>
            </label>
            <input
          ...
          <div className="bill-balance-row">
            ...
            <span
              className="bill-balance-value"
              style={{ color: balance > 0 ? 'var(--pp-danger-fg)' : 'var(--pp-success-fg)' }}
            >
            ...
            <CodeAutocomplete
              ...
              onSelect={(code) => {
                ...
                if (code)
                  set(
                    'treatment',
                    `[${(code as ProcedureCodeResult).code}] ${(code as ProcedureCodeResult).name}`,
                  );
              }}
            />
            {!selectedProcedure && (
            ...
            <CodeAutocomplete
              ...
              onSelect={(code) => {
                ...
                if (code)
                  set(
                    'disease',
                    `[${(code as IcdCodeResult).code}] ${(code as IcdCodeResult).description}`,
                  );
              }}
            />
            {!selectedIcd && (
      ...
      <div
        className="bill-form-section"
        style={{ display: 'flex', gap: 'var(--pp-space-3)', justifyContent: 'flex-end' }}
      >
        <button
          type="button"
          className="bill-btn"
          onClick={() => (onCancel ? onCancel() : navigate('/billing'))}
        >
          Cancel
        </button>
        <button
      ...
      <div className="bill-header">
        <div>
          ...
          <p className="bill-header-sub">
            Fill in treatment charges and payment details for official records.
          </p>
        </div>
