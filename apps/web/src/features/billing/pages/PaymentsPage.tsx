...
export default function PaymentsPage() {
  ...
  const totalCollected = filtered.reduce(
    (s, p) => (p.status === 'Success' || p.status === 'Completed' ? s + p.amount : s),
    0,
  );
  const totalPending = filtered.reduce((s, p) => (p.status === 'Pending' ? s + p.amount : s), 0);
  const avgTicket = filtered.length
    ? Math.round(
        totalCollected /
          filtered.filter((p) => p.status === 'Success' || p.status === 'Completed').length || 0,
      )
    : 0;
    ...
    <div className="pp-page-container bill-page animate-fade-in">
      ...
      <div className="pp-page-hero">
        <div>
          ...
          <p className="bill-header-sub">
            Transaction history, counter receipts and payment mode breakdown.
          </p>
        </div>
      ...
      {historyQuery.isLoading ? (
        ...
        <EmptyState
          icon={Banknote}
          title={
            regidFilter || fromDate || toDate ? 'No matches found' : 'No payment records found'
          }
          description={
            regidFilter || fromDate || toDate
              ? `No transactions matching your search filters were found.`
              : 'The ledger is empty. Record a manual counter payment or check for online transactions.'
          }
                ...
                {filtered.map((payment) => (
                  <tr key={payment.id}>
                    ...
                    <td data-label="Patient">
                      <div className="plat-cell-val">
                        <div
                          style={{ fontWeight: 600, fontSize: '0.875rem', color: 'var(--pp-ink)' }}
                        >
                          {payment.patientName || '—'}
                        </div>
                        {payment.regid && (
                    ...
                    <td data-label="Mode">
                      <div className="plat-cell-val">
                        <span
                          className={`bill-badge ${MODE_CLASS[payment.paymentMode] ?? 'bill-badge-default'}`}
                    ...
                    <td data-label="Reference">
                      <div className="plat-cell-val">
                        ...
                        >
                          {payment.orderId && (
                            <div title={payment.orderId}>ORD: {payment.orderId.slice(-8)}</div>
                          )}
                          {payment.paymentId && (
                            <div title={payment.paymentId}>PAY: {payment.paymentId.slice(-8)}</div>
                          )}
                          {!payment.orderId && !payment.paymentId && (
                            <span
                              className="bill-badge bill-badge-default"
                    ...
                    <td data-label="Status">
                      <div className="plat-cell-val">
                        <span
                          className={`bill-badge ${STATUS_CLASS[payment.status] ?? 'bill-badge-default'}`}
                          style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}
                        >
...
function ManualPaymentDrawerContent({ onClose }: { onClose: () => void }) {
  ...
  } = usePatient(regid ? parseInt(regid) : 0);
