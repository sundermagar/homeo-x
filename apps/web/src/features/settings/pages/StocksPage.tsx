        ...
        {isLoading ? (
          ...
          <div className="plat-table-container">
            <table className="plat-table">
              ...
              <tbody>
                {filtered.map((item: Stock, idx: number) => (
                  <tr key={item.id} className="plat-table-row">
                    <td className="plat-table-cell font-mono text-xs opacity-50">{idx + 1}</td>
        ...
        <div className="plat-modal-backdrop" onClick={() => setIsModalOpen(false)}>
          <div className="plat-modal-content max-w-2xl" onClick={e => e.stopPropagation()}>
            <div className="plat-modal-header">
              <h2 className="plat-modal-title">{editingId ? 'Edit Stock Item' : 'Add New Stock'}</h2>
