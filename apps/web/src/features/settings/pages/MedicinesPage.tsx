...
import { Pill, Plus, X, RefreshCw, Trash2, Edit2, Info, Package, IndianRupee, Search, AlertCircle, CheckCircle2  } from 'lucide-react';
            ...
            <table className="plat-table">
              ...
              <tbody>
                {filtered.map((med: Medicine, idx: number) => {
                   ...
                   return (
                    <tr key={med.id}>
                      <td data-label="#" style={{ color: 'var(--pp-text-3)', fontFamily: 'var(--pp-font-mono)', fontSize: '0.78rem' }}>{idx + 1}</td>
                      <td data-label="Medicine">
                        <div>
                          <div style={{ fontWeight: 700 }}>{med.name}</div>
                          <div style={{ fontSize: '0.75rem', color: 'var(--pp-text-3)', marginTop: 2, display: 'flex', alignItems: 'center', gap: 4 }}>
                      ...
                      <td data-label="Category">
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                          {med.category && <span className="plat-badge plat-badge-staff">{med.category}</span>}
                          {med.type && <span className="plat-badge plat-badge-default">{med.type}</span>}
                          <span className="plat-badge plat-badge-default" style={{ fontStyle: 'italic' }}>{potencyName}</span>
                        </div>
                      </td>
                      <td data-label="Stock">
                        <div>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontFamily: 'var(--pp-font-mono)', fontSize: '0.875rem' }}>
                            <Package size={12} style={{ color: 'var(--pp-text-3)' }} />
                            <span style={{ color: isOutOfStock ? 'var(--pp-danger-fg)' : isLowStock ? 'var(--pp-warning-fg)' : 'var(--pp-success-fg)', fontWeight: isOutOfStock || isLowStock ? 700 : 400 }}>
                              {stock} units
                            </span>
                          </div>
                          {isOutOfStock ? (
                            <span className="plat-badge plat-badge-danger" style={{ marginTop: 4 }}>Out of Stock</span>
                          ) : isLowStock ? (
                            <span className="plat-badge plat-badge-warning" style={{ marginTop: 4 }}>Low Stock</span>
                          ) : (
                            <span className="plat-badge plat-badge-staff" style={{ marginTop: 4 }}>Available</span>
                          )}
                        </div>
                      </td>
                      <td data-label="Price">
                        <div style={{ display: 'flex', alignItems: 'center', gap: 4, fontFamily: 'var(--pp-font-mono)', fontWeight: 600, color: 'var(--pp-blue)' }}>
                          <IndianRupee size={13} />{med.price || 0}
                        </div>
                      </td>
                      <td>
                        <div className="plat-action-group">
                          <button className="plat-btn plat-btn-sm plat-btn-icon" onClick={() => handleOpenEdit(med)} title="Edit">
                            <Edit2 size={13} />
                          </button>
