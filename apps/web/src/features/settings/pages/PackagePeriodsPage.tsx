...
export default function PackagePeriodsPage() {
  ...
  const filteredItems = periods.filter(
    (p: any) =>
      ...
      <div className="plat-header">
        <div>
          ...
          <p className="plat-header-sub">
            Predefined membership plan durations and effective validity days.
          </p>
        </div>
                ...
                <tbody>
                  {paginatedData.map((period: any, idx: number) => (
                    <tr key={period.id} className="plat-table-row">
                      <td
                        data-label="ID"
                        className="plat-table-cell font-mono text-xs color-muted text-left"
                      >
                        {(currentPage - 1) * itemsPerPage + idx + 1}
                      </td>
                      <td
                        data-label="Period Name"
                        className="plat-table-cell font-semibold text-left"
                      >
                        {period.name}
                      </td>
                      <td
                        data-label="Description"
                        className="plat-table-cell text-left color-muted text-xs"
                      >
                        {period.description || 'No description'}
                      </td>
                      <td
                        data-label="Effective Days"
                        className="plat-table-cell font-mono font-bold text-left"
                      >
                      ...
                      <td data-label="Status" className="plat-table-cell text-left">
                        <span
                          className={`plat-badge ${period.isActive ? 'plat-badge-staff' : 'plat-badge-default'}`}
                        >
