...
export default function StocksLogPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMsg(
        `Stock ${form.changeType === 'INVENTORY_ADD' ? 'added' : 'deducted'} for "${med?.name || 'medicine'}" successfully.`,
      );
      setForm(EMPTY_FORM);
      setTimeout(() => {
  ...
  const totalIn = (logs as any[])
    .filter((l) => l.changeType === 'INVENTORY_ADD')
    .reduce((s: number, l: any) => s + Number(l.quantity), 0);
  const totalOut = (logs as any[])
    .filter((l) => l.changeType !== 'INVENTORY_ADD')
    .reduce((s: number, l: any) => s + Number(l.quantity), 0);
    ...
    <div className="plat-page fade-in">
      <div className="plat-header">
        <div>
          ...
          <p className="plat-header-sub">
            Track inventory movements — add or deduct stock for medicines in the catalog.
          </p>
        </div>
      ...
      <div className="plat-stats-bar">
        ...
        <div className="plat-stat-card">
          ...
          <p className="plat-stat-value" style={{ color: 'var(--pp-success-fg)' }}>
            +{totalIn}
          </p>
        </div>
        <div className="plat-stat-card">
          <p className="plat-stat-label">Stock Out</p>
          <p className="plat-stat-value" style={{ color: 'var(--pp-danger-fg)' }}>
            -{totalOut}
          </p>
        </div>
      </div>
      ...
      <div className="plat-filters">
        <div className="plat-search-wrap">
          ...
          <select
            ...
            onChange={(e) =>
              setSelectedMedicine(e.target.value ? Number(e.target.value) : undefined)
            }
          >
                      ...
                      <tr key={log.id} className="plat-table-row">
                        ...
                        >
                          <div className="plat-stock-info">
                            <div
                              style={{ fontWeight: 600, fontSize: '14px', color: 'var(--pp-ink)' }}
                            >
                        ...
                        >
                          <div className="plat-stock-badge-wrap">
                            <span
                              className={`plat-badge ${isPositive ? 'plat-badge-staff' : ''}`}
                              style={
                                isPositive
                                  ? {
                                      display: 'flex',
                        ...
                        <td data-label="QTY" className="plat-table-cell" style={{ width: '100px' }}>
                          <div
                            style={{
                              fontFamily: 'monospace',
                              fontWeight: 600,
                              color: isPositive ? 'var(--pp-success-fg)' : 'var(--pp-danger-fg)',
                              fontSize: '15px',
                            }}
                          >
                            {isPositive ? '+' : '-'}
                        ...
                        >
                          ...
                          >
                            <span style={{ color: 'var(--pp-text-3)' }}>
                              {log.previousStock ?? '—'}
                            </span>
                            <span style={{ opacity: 0.3, color: 'var(--pp-ink)' }}>→</span>
                            <span style={{ fontWeight: 600, color: 'var(--pp-ink)' }}>
                              {log.newStock ?? '—'}
                            </span>
                            <span
                              style={{ fontSize: '9px', color: 'var(--pp-text-3)', opacity: 0.6 }}
                            >
                              units
                            </span>
                          </div>
          ...
          <div className="plat-modal-body" style={{ padding: 0 }}>
            ...
            >
              <CustomSearchSelect
                label="Medicine *"
                value={
                  (medicines as any[]).find((m) => m.id === Number(form.medicineId))?.name || ''
                }
