...
import { useCreateCustomBill, useRecordPayment } from '../../billing/hooks/use-billing';
...
export function BillingUpdateModal({
  ...
  const [paymentMode, setPaymentMode] = useState<
    'Cash' | 'Card' | 'Cheque' | 'UPI' | 'Online' | 'Bank Transfer'
  >('Cash');
  ...
  React.useEffect(() => {
    if (activeTab === 'custom' && todayCharges.length > 0 && !editingChargeId) {
      ...
      setAmount(
        (firstCharge.price || firstCharge.additionalPrice || firstCharge.amount || 0).toString(),
      );

      const match = chargesCatalog.find((c) => c.charges === title);
      const isProd =
        (match && match.type === 'Product') ||
        (firstCharge.quantity !== undefined && firstCharge.quantity !== null);
  ...
  const isLoading =
    ...
    createAdditionalCharge.isPending ||
    updateAdditionalCharge.isPending ||
  ...
  const maxQuantity =
    selectedCatalogItem?.type === 'Product' ? selectedCatalogItem.quantity || 0 : null;
        ...
        >
          <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
            ...
            <div style={{ fontSize: '0.8rem', color: '#64748b', fontWeight: 500 }}>
              Patient #{regid} •{' '}
              <span style={{ color: '#0f172a', fontWeight: 700 }}>{patientName}</span>
            </div>
          </div>
          <button
            onClick={onClose}
              ...
              {chargesCatalog && chargesCatalog.length > 0 && (
                <div>
                  ...
                  <select
                    ...
                    value={
                      chargesCatalog.find((c) => c.charges === customTitle)?.id?.toString() || ''
                    }
                    onChange={(e) => {
                  ...
                  >
                    <option value="" disabled>
                      -- Choose predefined service or product --
                    </option>
              ...
              {isProduct && (
                <div>
                    ...
                    {maxQuantity !== null && (
                      <span
                        style={{
                          fontSize: '0.75rem',
                          fontWeight: 600,
                          color: maxQuantity > 0 ? '#64748b' : '#ef4444',
                        }}
                      >
                  ...
                  {maxQuantity !== null && quantity > maxQuantity && (
                    ...
                    >
                      <AlertCircle size={14} /> Selected quantity ({quantity}) exceeds available
                      stock ({maxQuantity})!
                    </div>
                  )}
                </div>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: '0.75rem', color: '#64748b', marginTop: '2px' }}>
                        {ac.quantity && ac.quantity > 1
                          ? `₹${ac.price} × ${ac.quantity}`
                          : `₹${ac.price || ac.amount}`}
                        {ac.createdAt &&
                          ` • ${new Date(ac.createdAt).toLocaleDateString(undefined, { day: 'numeric', month: 'short' })}`}
                      </div>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                      ...
                      <button
                        onClick={(e) => {
                          ...
                          const isProd =
                            (match && match.type === 'Product') ||
                            (ac.quantity !== undefined && ac.quantity !== null);
