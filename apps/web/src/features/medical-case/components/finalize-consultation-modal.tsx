...
export function FinalizeConsultationModal({
  ...
  const handleFinalize = async () => {
    ...
    try {
      ...
      const mappedPrescriptions = itemsToBill
        .map((p) => {
          const medicine = lookups?.medicines?.find(
            (m) => m.name.toLowerCase() === p.remedy_name?.toLowerCase(),
          );
          return {
    ...
    <>
      ...
      <div className="mc-drawer">
        <div className="mc-drawer-header">
          ...
          <button className="mc-drawer-close" onClick={onClose}>
            <X size={16} />
          </button>
        </div>

        <div
          style={{ padding: '16px 20px', background: 'white', borderBottom: '1px solid #e2e8f0' }}
        >
          <div
            style={{ fontSize: '0.8rem', color: '#64748b', fontWeight: 600, marginBottom: '2px' }}
          >
            SESSION END
          </div>
          <div style={{ fontSize: '1rem', fontWeight: 700, color: '#1e293b' }}>
            Confirm charges & complete session
          </div>
        </div>
        ...
        <div className="mc-drawer-body" style={{ flex: 1, padding: '24px' }}>
          {activePackageName && (
            ...
            >
              ...
              >
                <strong>Medicine charges</strong> are automatically waived due to active{' '}
                <strong>{activePackageName}</strong> plan.
              </p>
            </div>
          )}
          ...
          <div className="mc-legacy-input-group">
            ...
            >
              Note: This will automatically generate a bill for the front-office team and mark this
              visit as completed.
            </p>
          </div>
        </div>
        ...
        >
          <button className="mc-legacy-btn-secondary" style={{ flex: 1 }} onClick={onClose}>
            Discard
          </button>
          <button
