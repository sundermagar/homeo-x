    ...
    <>
      {parts.map((part, i) =>
        part.toLowerCase() === query.toLowerCase() ? (
          <span key={i} className="mc-highlight">
            {part}
          </span>
        ) : (
          part
...
function MatrixNode({
  ...
  const { data: alternatives, isLoading: loadingAlts } = useRemedyAlternatives(
    isOpen ? node.id : null,
  );
      ...
      >
        ...
        {depth > 0 && (
          <button
            onClick={(e) => {
              e.stopPropagation();
              handlePrescribe(node.label, null);
            }}
      ...
      {isOpen && (
        ...
        >
          ...
          {alternatives && alternatives.length > 0 && (
            <div
              style={{ padding: '8px', background: 'var(--bg-surface-2)', borderRadius: '10px' }}
            >
              <div className="mc-count" style={{ marginBottom: '8px', fontSize: '0.62rem' }}>
...
export function AiRemedyView({ regid }: { regid?: number }) {
  ...
  const { data: searchResults, isLoading: loadingSearch } = useRemedyTree(
    searchTerm ? 0 : -1,
    searchTerm,
  );
    ...
    >
      ...
      <div className="mc-detail-header" style={{ marginBottom: '4px', padding: '10px 16px' }}>
        <div className="mc-back-group">
          <div>
            <h1 className="mc-page-title" style={{ fontSize: '1.15rem', marginBottom: '0' }}>
              Remedy Explorer
            </h1>
            <p className="mc-page-sub" style={{ fontSize: '0.65rem' }}>
              Taxonomically mapped Materia Medica
            </p>
          </div>
        </div>
        {regid && (
          <span className="mc-active-badge" style={{ padding: '2px 6px', fontSize: '0.6rem' }}>
            Active Session: #{regid}
          </span>
        )}
      </div>

      <div
        className="mc-matrix-layout"
        style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}
      >
        {/* ── Left Column: Matrix Explorer ── */}
        <div
          className="mc-matrix-main"
          style={{
            display: 'flex',
            flexDirection: 'column',
            background: 'var(--bg-card)',
            ...
            padding: '0',
            border: '1px solid var(--border-main)',
            boxShadow: 'var(--pp-premium-shadow)',
            overflow: 'hidden',
            height: '100%',
          }}
        >
          <div
            className="mc-remedy-header"
            style={{ padding: '10px 16px', borderBottom: '1px solid var(--border-main)' }}
          >
            <div>
              <h2 className="mc-title" style={{ fontSize: '1rem', fontWeight: 800 }}>
                Pharmacopoeia & Taxonomy Matrix
              </h2>
              <p
                className="mc-subtitle"
                style={{ fontSize: '0.65rem', color: 'var(--text-muted)' }}
              >
                Hierarchical mapping of remedies
              </p>
            </div>
            {selectedPath && (
          ...
          >
            {isLoading ? (
              ...
              <div className="mc-empty-state">
                ...
                <p>
                  {searchTerm ? 'No matches found in the matrix.' : 'No taxonomy data available.'}
                </p>
              </div>
            ) : (
              <>
