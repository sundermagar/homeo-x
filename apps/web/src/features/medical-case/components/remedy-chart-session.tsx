...
export function RemedyChartSession({
  ...
  const handleDelete = async (id: number, name?: string) => {
    ...
    try {
      ...
      alert(
        `Failed to remove prescription: ${err?.response?.data?.message || err?.message || 'Unknown error'}`,
      );
    }
  };
  ...
  const handlePrintRow = (rx: PrescriptionRow) => {
    ...
    const queryStr = dateParam
      ? `&date=${encodeURIComponent(new Date(dateParam).toISOString())}`
      : '';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', gap: '16px' }}>
      ...
      >
        <div className="mc-action-bar">
          ...
          >
            ...
            <div
              className="mc-service-indicator"
              style={{
                width: '100%',
                minWidth: 0,
            ...
            >
              ...
              >
                ...
                {delivery === 'pickup' && (
                  <>
                    <Package size={14} /> <span>pickup</span>
                  </>
                )}
              </div>
            </div>
            ...
            <>
              ...
              >
                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                  <label style={{ fontSize: '0.8rem', fontWeight: 700, color: 'var(--pp-ink)' }}>
                    Remedy:
                  </label>
                  <SearchableSelect
                ...
                {(!isRxToday ||
                  (editingId && firstRxOfToday && editingId === firstRxOfToday.id)) && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                    <label style={{ fontSize: '0.8rem', fontWeight: 700, color: 'var(--pp-ink)' }}>
                      Days:
                    </label>
          ...
          >
            <div
              style={{
                ...
                borderBottom: '1px solid var(--pp-blue-border)',
                display: 'flex',
                alignItems: 'center',
                gap: '8px',
              }}
            >
              ...
              <span style={{ fontSize: '0.85rem', fontWeight: 800, color: 'var(--pp-blue)' }}>
                ...
                <tbody>
                  ...
                  {groupedHistory.map((group) => (
                    <React.Fragment key={group.date}>
                      {group.items.map((rx, idx) => {
                        ...
                        const isRowSelected =
                          selectedDate &&
                          new Date(rx.created_at || rx.createdAt || rx.dateval).toDateString() ===
                        ...
                        return (
                          ...
                          >
                            <td data-label="Date">
                              {idx === 0 ? (
                                ...
                                >
                                  ...
                                  >
                                    {new Date(
                                      rx.created_at || rx.createdAt || rx.dateval,
                                    ).getDate()}
                                  </span>
                                  <span
                                    style={{
                            ...
                            <td data-label="Remedy">
                              <div className="remedy-name">{rx.remedy_name}</div>
                            </td>
                            ...
                            <td data-label="Frequency">
                              <span
                                style={{ color: '#7c3aed', fontWeight: 700, fontSize: '0.9rem' }}
                              >
                                {rx.frequency_name}
                              </span>
                            </td>
                            ...
                            <td data-label="Instructions" className="mc-col-instructions">
                              <div
                                style={{
                                  ...
                                  paddingRight: '8px',
                                  background:
                                    rx.prescription || rx.notes ? '#f8fafc' : 'transparent',
                                  borderRadius: '6px',
                              ...
                              <div className="mc-table-actions">
                                <div className="mc-desktop-actions">
                                  {(() => {
                                    ...
                                    const isToday =
                                      rxDate.toDateString() === new Date().toDateString();
                                    return (
                                      <>
                                        <button
                                          onClick={(e) => {
                                        ...
                                        {isToday && (
                                          <>
                                            ...
                                            <button
                                              onClick={(e) => {
                                                e.stopPropagation();
                                                handleDelete(rx.id, rx.remedy_name);
                                              }}
                                              className="mc-action-btn danger"
                                              title="Remove"
                                            >
                                              <Trash2 size={14} />
                                            </button>
                  ...
                  {history?.length === 0 && (
                    <tr>
                      <td
                        colSpan={7}
                        style={{ padding: '24px', color: 'var(--pp-text-3)', textAlign: 'center' }}
                      >
                        No previous prescriptions found for this patient. Add a new prescription
                        above.
                      </td>
                    </tr>
                  )}
      ...
      {activeTab === 'image' && <ImageUploadTab regid={Number(regid)} />}

      {/* Premium Confirmation Dialog */}
      <Dialog open={showConfirm} onOpenChange={setShowConfirm}>
        <DialogContent
          className="premium-alert-content"
          style={{ maxWidth: '440px', borderRadius: '24px', padding: '32px' }}
        >
          <DialogHeader
            className="premium-alert-header"
          ...
          >
            ...
            <DialogDescription
              className="premium-alert-description"
              style={{ fontSize: '1rem', lineHeight: 1.6, color: 'var(--pp-text-3)' }}
            >
              {isRxToday ? (
                <>
                  A follow-up for <strong style={{ color: '#f59e0b' }}>today</strong> has already
                  been recorded.
                  <br />
                  To add additional remedies to this session, please use the{' '}
                  <strong style={{ color: 'var(--pp-blue)' }}>Add Extra</strong> button.
                </>
              ) : (
                <>
          ...
          <DialogFooter
            className="premium-alert-footer"
            style={{ marginTop: '32px', gap: '12px', justifyContent: 'center' }}
          >
            {isRxToday ? (
              <button
        ...
        >
          ...
          >
            ...
            >
              ...
              <br />
              <br />
              Starting a "Repeat" session creates a fresh follow-up. Since today's follow-up is
              already recorded, please use the{' '}
              <strong style={{ color: 'var(--pp-blue)' }}>Add Extra</strong> button to add more
              remedies to the current session.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter
            className="premium-alert-footer"
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span>
          These files will also appear in the <strong>Media</strong> tab of the patient record.
        </span>
      </div>
    </div>
  );
