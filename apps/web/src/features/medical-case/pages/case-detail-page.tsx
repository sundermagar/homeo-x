  ...
  if (isFirst) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
        ...
        >
          {new Date(dateVal).toLocaleString('default', { month: 'short' })}{' '}
...
export function AutoSaveNoteArea({
  value,
  onChange,
  onSave,
  placeholder = '',
...
export default function MedicalCaseDetailPage() {
  ...
  useEffect(() => {
    ...
    // Join case room
    socket.emit('join-case', Number(regid));

    const handleMediaUpdate = (data: { regid: number; action: string }) => {
      if (Number(data.regid) === Number(regid)) {
        queryClient.invalidateQueries({ queryKey: ['medical-case', 'full', Number(regid)] });
      }
    };

    socket.on('case:media-updated', handleMediaUpdate);

    return () => {
      socket.emit('leave-case', Number(regid));
      socket.off('case:media-updated', handleMediaUpdate);
    };
  }, [regid, queryClient]);
  ...
  const [activeBillingTab, setActiveBillingTab] = useState<'regular' | 'custom' | 'payment'>(
    'regular',
  );
  ...
  const {
    ...
    error,
    refetch: refetchFull,
  } = useFullMedicalCase(Number(regid));
  ...
  const [mobileDrawer, setMobileDrawer] = useState<
    'followup' | 'billing' | 'contact' | 'package' | null
  >(null);
  ...
  const { finalizeConsultation, saveNote, updateDiagnosis, saveSoap, deleteRecord } =
    useManageClinicalRecords();
  ...
  useEffect(() => {
    if (prescriptionsHistory?.length && !selectedDate) {
      const sorted = [...prescriptionsHistory].sort(
        (a, b) =>
          new Date(b.created_at || b.dateval).getTime() -
          new Date(a.created_at || a.dateval).getTime(),
      );
      if (sorted[0]) {
    ...
    try {
      ...
      alert(
        err instanceof Error
          ? err.message
          : 'Failed to scan the prescription image. Please try again.',
      );
    } finally {
    ...
    try {
      if (scannedPrescription.diagnosis.trim()) {
        await updateDiagnosis.mutateAsync({
          regid: Number(regid),
          condition: scannedPrescription.diagnosis.trim(),
        });
      }
      const serializedMeds = JSON.stringify(
        scannedMedicationRows.filter((r) => r.medicine.trim() !== ''),
      );
  ...
  const detectMedicineIssue = async (index: number, medicineName: string) => {
    ...
    try {
      const res = await apiClient.post<{
        success: boolean;
        data: { issue: string; provider?: string };
      }>('/medical-cases/ai-detect-medicine-issue', { medicine: medicineName.trim() });
  ...
  const handleSaveDiagnosis = async () => {
    try {
      ...
      if (diagForm.diagnosis.trim()) {
        await updateDiagnosis.mutateAsync({
          regid: Number(regid),
          condition: diagForm.diagnosis.trim(),
        });
      }
  ...
  const followupNotes = React.useMemo(() => {
    return (notes || [])
      ...
      .sort(
        (a: any, b: any) =>
          new Date(b.createdAt || b.created_at || b.dateval || 0).getTime() -
          new Date(a.createdAt || a.created_at || a.dateval || 0).getTime(),
      );
  }, [notes]);
  ...
  const latestPrescriptionDateStr =
    latestPrescriptionObj?.created_at || latestPrescriptionObj?.dateval;
  const latestNoteDate = followupNotes[0]
    ? parseSafeDate(
        followupNotes[0].createdAt || followupNotes[0].created_at || followupNotes[0].dateval,
      )
    : null;
  const latestRxDate = parseSafeDate(latestPrescriptionDateStr);

  const defaultEncounterDate =
  ...
  const billingValues = useMemo(() => {
    ...
    const currentPaid = dayBills.reduce(
      (sum: number, b: any) => sum + (Number(b.received) || 0),
      0,
    );
    ...
    const effectiveDaysCharge =
      hasActivePackage && fullData?.activePackage?.coversMedicine && !isCompleted
        ? 0
  ...
  const currentVisitId = useMemo(() => {
    ...
    const idFromRx = rx
      ? (rx.visitId ?? rx.visit_id ?? rx.consultationId ?? rx.consultation_id)
      : null;
  ...
  const tabContent = useMemo(() => {
    ...
    const filteredInvestigations = displayDate
      ? filterByDate(investigations, displayDate)
      : investigations;
    ...
    switch (activeTab) {
        ...
        return (
          <div className="mc-tab-content-wrapper">
            <VitalsView
              ...
              phone={medicalCase.phone || medicalCase.mobile || ''}
      ...
      case 'vaccine':
        return (
          <div className="mc-tab-content-wrapper">
            <VaccineView regid={Number(regid)} caseVaccines={vaccines} onAppendNote={appendNote} />
          </div>
        );
      case 'analytics':
        return (
      ...
      case 'reports':
        return (
          <div className="mc-tab-content-wrapper">
            <ReportsView regid={Number(regid)} investigations={filteredInvestigations} />
          </div>
        );
      case 'ai-assist':
        return (
          <div className="mc-tab-content-wrapper">
            <AiConsultantView regid={Number(regid)} />
          </div>
        );
      default:
        return (
        ...
        <div className="profile-bottom-grid">
          ...
          <div className="profile-info-cell">
            ...
            <span>
              {(() => {
                ...
                if (!regDate) return '—';
                return new Date(regDate).toLocaleDateString('en-GB', {
                  day: '2-digit',
                  month: 'short',
                  year: 'numeric',
                });
          ...
          {activePackage?.expiryDate && (
            <div className="profile-info-cell">
              ...
              <div className="info-with-icon">
                <Clock size={14} />{' '}
                {new Date(activePackage.expiryDate).toLocaleDateString('en-GB', {
                  day: '2-digit',
                  month: 'short',
                })}
              </div>
        ...
        <div className="mc-body-grid">
            ...
            {activeTab === 'summary' && (
              ...
              >
                ...
                {billingValues.hasActivePackage && (
                  <div
                    style={{
                      background: `${billingValues.activePackageColor || '#3b82f6'}1a`,
                      border: `1px solid ${billingValues.activePackageColor || '#3b82f6'}33`,
                      padding: '8px 16px',
                      borderRadius: '8px',
                      display: 'flex',
                      alignItems: 'center',
                  ...
                  >
                    ...
                    <span
                      style={{
                        fontSize: '0.75rem',
                        fontWeight: 700,
                        color: billingValues.activePackageColor || '#1e40af',
                      }}
                    >
                  ...
                  {[
                    {
                      ...
                      value: billingValues.regular,
                      color: '#1e293b',
                      tab: 'regular',
                      isCovered:
                        billingValues.hasActivePackage &&
                        billingValues.originalRegular > 0 &&
                        billingValues.regular === 0,
                      originalValue: billingValues.originalRegular,
                    },
                    {
                      label: 'Medicine Days Charge',
                      value: billingValues.daysCharge,
                      color: '#475569',
                      tab: 'regular',
                      isCovered:
                        billingValues.hasActivePackage &&
                        billingValues.originalDaysCharge > 0 &&
                        billingValues.daysCharge === 0,
                      originalValue: billingValues.originalDaysCharge,
                    },
                    {
                      label: 'Additional Charge',
                      value: billingValues.additional,
                      color: '#64748b',
                      tab: 'custom',
                    },
                    {
                      label: 'Total Bill Amount',
                      value: billingValues.total,
                      color: '#7c3aed',
                      bold: true,
                      tab: 'regular',
                    },
                    {
                      ...
                      value: billingValues.received,
                      color: '#059669',
                      tab: 'payment',
                    },
                    {
                      label: 'Pending Balance',
                      value: billingValues.balance,
                      color: '#dc2626',
                      bold: true,
                      noEdit: true,
                    },
                    ...
                    >
                      ...
                      >
                        {row.isCovered ? (
                          <>
                            <del
                              style={{ color: '#94a3b8', fontSize: '0.75rem', marginRight: '6px' }}
                            >
                              ₹{row.originalValue}
                            </del>
          ...
          <aside className="mc-body-side">
            ...
            <div className="mc-side-card" style={{ marginBottom: '16px' }}>
              ...
              <div className="mc-side-card-body" style={{ padding: '16px' }}>
                <AutoSaveNoteArea
                  ...
                  placeholder={
                    displayDate ? 'Record patient follow-up or status...' : 'No history available'
                  }
            ...
            <div className="mc-side-card" style={{ marginBottom: '16px' }}>
              ...
              <div
                className="mc-side-card-body custom-scrollbar"
                style={{
                  padding: '16px',
                  display: 'flex',
                  flexDirection: 'column',
              ...
              >
                ...
                <div>
                  ...
                  <div style={{ fontSize: '0.85rem', color: '#475569', lineHeight: 1.5 }}>
                    {currentVisitSoap?.plan || currentVisitSoap?.advice || '—'}
                  </div>
                </div>
              </div>
            </div>
                ...
                <>
                  ...
                  <div className="mc-drawer animate-slide-in-right" style={{ maxWidth: '520px' }}>
                    <header
                      ...
                      style={{ background: 'var(--pp-blue)', color: 'white' }}
                    >
                      <div className="mc-drawer-header-title">
                        <Sparkles size={18} />{' '}
                        {editingDiagnosisRecord ? 'Edit AI Follow up' : 'New AI Follow up'}
                      </div>
                      <button
                    ...
                    >
                      ...
                      >
                        ...
                        <span
                          style={{ fontSize: '0.8rem', fontWeight: 700, color: 'var(--pp-ink)' }}
                        >
                          Record Date:{' '}
                              ...
                              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                ...
                                >
                                  ...
                                  <input
                                    ...
                                    value={row.medicine}
                                    onChange={(e) =>
                                      updateMedicationRow(idx, 'medicine', e.target.value)
                                    }
                                    style={{
                                      width: '100%',
                                      padding: '8px 12px',
                                ...
                                >
                                  ...
                                  >
                                    ...
                                    <select
                                      ...
                                      value={row.frequency}
                                      onChange={(e) =>
                                        updateMedicationRow(idx, 'frequency', e.target.value)
                                      }
                                      style={{
                                        width: '100%',
                                        padding: '8px',
                                  ...
                                  >
                                    ...
                                    >
                                      ...
                                      <button
                                        ...
                                        style={{
                                          ...
                                          border: '1px solid',
                                          borderColor:
                                            aiDetectingIdx === idx ? '#e2e8f0' : '#c4b5fd',
                                          borderRadius: '6px',
                                          padding: '3px 8px',
                                          cursor:
                                            !row.medicine.trim() || aiDetectingIdx === idx
                                              ? 'not-allowed'
                                              : 'pointer',
                                          transition: 'all 0.2s ease',
                                    ...
                                    <input
                                      ...
                                      value={row.issue}
                                      onChange={(e) =>
                                        updateMedicationRow(idx, 'issue', e.target.value)
                                      }
                                      style={{
                                        width: '100%',
                                        padding: '8px 12px',
              ...
              ReactDOM.createPortal(
                <>
                  ...
                  <div className="mc-drawer animate-slide-in-right" style={{ maxWidth: '540px' }}>
                    ...
                    >
                      <div
                        className="mc-drawer-header-title"
                              ...
                              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                ...
                                >
                                  ...
                                  <input
                                    ...
                                    value={row.medicine}
                                    onChange={(e) =>
                                      updateScannedMedicationRow(idx, 'medicine', e.target.value)
                                    }
                                    style={{
                                      width: '100%',
                                      padding: '8px 12px',
                                ...
                                >
                                  ...
                                  >
                                    ...
                                    <select
                                      ...
                                      value={row.frequency}
                                      onChange={(e) =>
                                        updateScannedMedicationRow(idx, 'frequency', e.target.value)
                                      }
                                      style={{
                                        width: '100%',
                                        padding: '8px',
                                  ...
                                  >
                                    ...
                                    <input
                                      ...
                                      value={row.issue}
                                      onChange={(e) =>
                                        updateScannedMedicationRow(idx, 'issue', e.target.value)
                                      }
                                      style={{
                                        width: '100%',
                                        padding: '8px 12px',
            ...
            <div className="mc-drawer-header">
              <div className="mc-drawer-header-title">
                {mobileDrawer === 'followup' && (
                  <>
                    <FileText size={18} /> Follow-up Notes
                  </>
                )}
                {mobileDrawer === 'billing' && (
                  <>
                    <CreditCard size={18} /> Billing Summary
                  </>
                )}
                {mobileDrawer === 'contact' && (
                  <>
                    <Phone size={18} /> Patient Contact
                  </>
                )}
                {mobileDrawer === 'package' && (
                  <>
                    <Package size={18} /> Package Info
                  </>
                )}
              </div>
              ...
              {mobileDrawer === 'followup' && (
                <>
                  <div style={{ maxHeight: '300px', overflowY: 'auto', marginBottom: '12px' }}>
                    {(notes || []).filter(
                      (n: any) => n.notesType === 'Followup' || n.noteType === 'Followup',
                    ).length > 0 ? (
                      (notes || [])
                        .filter((n: any) => n.notesType === 'Followup' || n.noteType === 'Followup')
                        .sort(
                          (a: any, b: any) =>
                            new Date(b.createdAt || b.created_at || b.dateval || 0).getTime() -
                            new Date(a.createdAt || a.created_at || a.dateval || 0).getTime(),
                        ...
                        .map((note: any) => (
                          ...
                          >
                            ...
                            >
                              {note.createdAt || note.created_at || note.dateval
                                ? new Date(
                                    note.createdAt || note.created_at || note.dateval,
                            ...
                            <div style={{ color: 'var(--pp-ink)', lineHeight: 1.5 }}>
                              {note.notes}
                            </div>
                          </div>
                        ))
                  ...
                  <div className="mc-side-card" style={{ background: 'var(--bg-card)' }}>
                    <div className="mc-side-card-body">
                      ...
                      >
                        ...
                        <strong style={{ color: 'var(--pp-success-fg)' }}>
                          ₹{billingValues.received}
                        </strong>
                      </div>
                      <div
                        style={{
                      ...
                      >
                        ...
                        <strong style={{ color: 'var(--pp-danger-fg)' }}>
                          ₹{billingValues.balance}
                        </strong>
                      </div>
                      <div
                        style={{
    ...
    <div className="mc-detail-container animate-fade-in" style={{ padding: '24px' }}>
        ...
        >
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <div key={i} style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              <div
                className="skeleton-box"
                style={{ width: '60px', height: '10px', opacity: 0.2, borderRadius: '4px' }}
              />
              <div
                className="skeleton-box"
                style={{ width: '100px', height: '14px', opacity: 0.4, borderRadius: '4px' }}
              />
            </div>
          ))}
        </div>
      ...
      <div className="mc-body-grid" style={{ marginTop: '32px' }}>
        <div className="mc-body-main">
          ...
          <div
            className="pp-card"
            style={{ padding: '32px', minHeight: '400px', marginBottom: '24px' }}
          >
            <div
              className="skeleton-box skeleton-text title"
              style={{ width: '30%', marginBottom: '32px' }}
            />
            <div
              className="skeleton-box"
          ...
          >
            <div
              className="skeleton-box skeleton-text title"
              style={{ width: '120px', height: '14px', marginBottom: '20px' }}
            />
            <div
              style={{
                display: 'flex',
  ...
  const flatGrouped = React.useMemo(() => {
    ...
    Object.values(catMap)
      ...
      .forEach((cat: any) => {
        ...
        (childMap[cat.id] || [])
          .sort((a: any, b: any) => (a.months || 0) - (b.months || 0))
          .forEach((c: any) => list.push(c));
      });
    // Orphans
    masterVaccines
      .filter((v: any) => v.parentId !== 0 && !catMap[v.parentId])
      .forEach((v: any) => list.push(v));
            ...
            <tbody>
              {currentVaccines.length === 0 && (
                <tr>
                  <td
                    colSpan={5}
                    style={{ textAlign: 'center', padding: '32px', color: 'var(--pp-text-3)' }}
                  >
              ...
              {currentVaccines.map((vac: any, idx: number) => {
                if (vac.isHeader) {
                  return (
                    <tr key={`hdr-${vac.id}`} style={{ background: '#f0f9ff' }}>
                      <td
                        colSpan={5}
                        style={{ padding: '10px 14px', borderBottom: '1px solid #dbeafe' }}
                      >
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                return (
                  <tr key={vac.id} className="hover-row">
                    ...
                    <td>
                      <div style={{ fontWeight: 700, fontSize: '0.9rem', color: 'var(--pp-ink)' }}>
                        {vac.label}
                      </div>
                      {vac.description && (
                        <div
                      ...
                      {isDone && doneRecord?.createdAt && (
                        ...
                        >
                          ✓ Given on{' '}
                          {new Date(doneRecord.createdAt).toLocaleDateString('en-GB', {
                            day: '2-digit',
                            month: 'short',
                            year: 'numeric',
                          })}
                          {doneRecord.notes &&
                          doneRecord.notes !== 'Administered' &&
                          doneRecord.notes !== 'Done'
                            ? ` — ${doneRecord.notes}`
                            : ''}
                        </div>
                      )}
                      ...
                      {vac.months !== null && vac.months !== undefined ? (
                        <span
                          style={{
                            ...
                            fontSize: '0.72rem',
                            fontWeight: 700,
                            background: vac.months === 0 ? '#f0fdf4' : '#eff6ff',
                            color: vac.months === 0 ? '#16a34a' : '#2563eb',
                            border: `1px solid ${vac.months === 0 ? '#bbf7d0' : '#bfdbfe'}`,
                          }}
                        >
...
function AnalyticsView({
  ...
  const handleCopyToFollowup = () => {
    ...
    onAppendNote(
      `VITALS - H/W (${new Date(latest.recordedAt).toLocaleDateString()}): ${parts.join(', ')}`,
    );
  };
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', flex: '1 1 180px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <span
                ...
                onClick={() => setHeightUnit((h) => (h === 'cm' ? 'in' : 'cm'))}
              >
            ...
            <input
              type="number"
              className="pp-input"
              style={{ width: '100%', borderRadius: '8px', padding: '10px 16px' }}
              value={hVal}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', flex: '1 1 180px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <span
                ...
                onClick={() => setWeightUnit((w) => (w === 'kg' ? 'lbs' : 'kg'))}
              >
      ...
      >
        <div className="pp-card" style={{ padding: '16px' }}>
          <h4 style={{ fontSize: '0.85rem', color: 'var(--pp-text-3)', marginBottom: '16px' }}>
            Weight Trend (kg)
          </h4>
...
function ReportsView({ regid, investigations }: { regid: number; investigations: any[] }) {
  const reports =
    investigations?.filter(
      (inv) =>
        inv.type === 'X-ray - CT - MRI' ||
        inv.type === 'USG Female' ||
        inv.type === 'USG Male' ||
        inv.type === 'Serology' ||
        inv.type === 'Semen Analysis',
    ) || [];
          ...
          {reports.map((report) => (
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                ...
                <span style={{ fontSize: '0.8rem', color: 'var(--pp-text-3)' }}>
                  {report.investDate ? new Date(report.investDate).toLocaleDateString() : ''}
                </span>
              </div>
              <div
                style={{
...
function SidebarIcon({ icon: Icon, active, onClick, title }: any) {
  return (
    <div
      className={`mc-legacy-sidebar-icon ${active ? 'active' : ''}`}
      onClick={onClick}
      title={title}
    >
...
function VitalsView({
  ...
  const {
    expandedDates,
    toggleDate,
    groupedData: groupedVitals,
  } = useTableGrouping(currentVitals, 'recordedAt');
  ...
  return (
    <div className="mc-vitals-workspace animate-fade-in">
      ...
      >
        <div className="mc-section-header" style={{ margin: 0 }}>
      ...
      <div className="mc-vitals-grid">
        <VitalCard
          ...
          value={latest ? `${latest.systolicBp}/${latest.diastolicBp}` : '-'}
          unit="mmHg"
          icon={Activity}
        ...
        <VitalCard
          label="Temperature"
          value={latest ? latest.temperatureF : '-'}
          unit="°F"
          icon={Thermometer}
          color="#f59e0b"
        />
      ...
      {vitals.length === 0 ? (
        ...
        <div style={{ marginTop: '32px' }}>
          <div
            className="pp-card pp-table-scroll"
            style={{ padding: 0, borderRadius: '12px', border: '1px solid #bfdbfe' }}
          >
            <div
              style={{
                padding: '12px 16px',
...
const LAB_CONFIG: Record<string, any[]> = {
  ...
  'X-ray - CT - MRI': [{ key: 'radiological_report', label: 'Radiological Reports', type: 'full' }],
  'USG Female': [
  ...
  Serology: [{ key: 'serological_report', label: 'Serological Reports', type: 'full' }],
  'Semen Analysis': [{ key: 'semen_analysis', label: 'Semen Analysis', type: 'full' }],
};
...
function LabsView({
  ...
  const handleCopyToFollowup = (inv: any) => {
    ...
    const summary = Object.entries(inv.data || {})
      ...
      .join(', ');
    onAppendNote(
      `INVESTIGATION (${inv.type} - ${new Date(inv.investDate).toLocaleDateString()}): ${summary}`,
    );
  };

  const sortedInvs = investigations
    ? [...investigations].sort(
        (a, b) => new Date(b.investDate).getTime() - new Date(a.investDate).getTime(),
      )
    : [];
  ...
  const {
    expandedDates,
    toggleDate,
    groupedData: groupedInvs,
  } = useTableGrouping(currentInvs, 'investDate');
  ...
  const handleDownload = () => {
    const content =
      `Investigation Report: ${activeType}\nDate: ${new Date().toLocaleDateString()}\n\n` +
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const { data } = await apiClient.post(
        '/medical-cases/records/investigations/upload',
        formData,
        {
          headers: {
  ...
  return (
    <div className="mc-labs-workspace animate-fade-in">
        ...
        <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
          ...
          >
            <button
              ...
              style={{
                ...
                color: viewMode === 'list' ? 'var(--pp-blue)' : '#64748b',
                border: 'none',
                cursor: 'pointer',
                fontSize: '0.85rem',
                fontWeight: 600,
            ...
            <button
              ...
              style={{
                ...
                boxShadow: viewMode === 'compare' ? '0 1px 2px rgba(0,0,0,0.1)' : 'none',
                color: viewMode === 'compare' ? 'var(--pp-blue)' : '#64748b',
                border: 'none',
                cursor: 'pointer',
                fontSize: '0.85rem',
                fontWeight: 600,
          ...
          >
            {isScanning ? (
              ...
              <Sparkles size={16} style={{ marginRight: '6px', color: '#8b5cf6' }} />
            )}
                        ...
                        {group.items.map((inv, idx) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <td style={{ fontWeight: 700, color: 'var(--pp-ink)' }}>
                                {inv.type}
                              </td>
                              <td>
                                <div
      ...
      {previewingInv && (
        <InvestigationPreviewModal inv={previewingInv} onClose={() => setPreviewingInv(null)} />
      )}
            ...
            <div className="mc-drawer animate-slide-in-right" style={{ maxWidth: '520px' }}>
                  ...
                  {fields.map((field) => (
                    <div
                      ...
                      style={{
                        ...
                        flexDirection: 'column',
                        gap: '6px',
                        ...(field.type === 'full' ? { gridColumn: '1 / -1' } : {}),
                      }}
                    >
                      <label
                        style={{
              ...
              <footer
                style={{
                  padding: '16px 24px',
                  background: 'var(--pp-warm-1)',
                  borderTop: '1px solid var(--pp-warm-3)',
                  display: 'flex',
                  gap: '10px',
                }}
              >
                ...
                {isScannedPreview ? (
                  ...
                  >
                    <Save size={16} />{' '}
                    {saved ? 'Saved!' : editingInv ? 'Update Report' : 'Save Report'}
                  </button>
                )}
              </footer>
            </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      <div className="pp-card" style={{ padding: '24px' }}>
        <div className="mc-section-header" style={{ marginBottom: '20px' }}>
          Message {name}
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div>
            ...
            <select className="pp-select" onChange={(e) => setMessage(e.target.value)} value="">
              ...
              <option value="Hello, Your medicine from Homeo-X is dispatched via courier. Tracking: ">
                Medicine Dispatched
              </option>
              <option value="Reminder: Your follow-up consultation is scheduled for tomorrow. Please confirm.">
                Follow-up Reminder
              </option>
              <option value="Hello, Your lab reports are ready. You can view them on the Homeo-X app.">
                Lab Reports Ready
              </option>
              <option value="Greeting from Homeo-X. How is your health today? Any improvements?">
                Health Check-in
              </option>
            </select>
          </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          {logs.map((log: any) => (
            ...
            >
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <div
                    style={{
                      ...
                      height: '8px',
                      borderRadius: '50%',
                      background: log.status === 'sent' ? '#10b981' : '#f59e0b',
                    }}
                  />
                  <span
                    style={{
              ...
              <p
                style={{ fontSize: '0.85rem', color: 'var(--pp-ink)', margin: 0, lineHeight: 1.5 }}
              >
                {log.message}
              </p>
            </div>
...
function MediaView({
  ...
  const {
    expandedDates,
    toggleDate,
    groupedData: groupedImages,
  } = useTableGrouping(currentImages, 'createdAt');
    ...
    <div className="animate-fade-in" style={{ maxWidth: '1000px', margin: '0 auto' }}>
      ...
      {showUploadForm && (
        ...
        >
                ...
                {file ? (
                  ...
                  <>
                    ...
                    <div>
                      ...
                      <div
                        style={{ color: 'var(--pp-text-3)', fontSize: '0.85rem', fontWeight: 500 }}
                      >
          ...
          <div
            style={{
              borderTop: '1px solid var(--pp-warm-2)',
              paddingTop: '16px',
              display: 'flex',
              alignItems: 'center',
              gap: '8px',
      ...
      {viewMode === 'grid' ? (
              ...
              >
                ...
                >
                  {resolvedUrl ? (
                          ...
                          <img
                            ...
                            onError={(e) => {
                              ...
                              el.parentElement!.innerHTML =
                                '<div style="text-align:center;color:#999;padding:20px"><div style="font-size:2rem;margin-bottom:8px">🖼️</div><div style="font-size:0.8rem">Image missing</div></div>';
                            }}
                          />
                      ...
                      {mediaType === 'video' && (
                        <video
                          ...
                          onError={(e) => {
                            ...
                            el.style.display = 'none';
                            el.parentElement!.innerHTML =
                              '<div style="text-align:center;color:#999;padding:20px"><div style="font-size:2rem;margin-bottom:8px">🎥</div><div style="font-size:0.8rem">Video format unsupported</div></div>';
                          }}
                        />
                      )}
                      {mediaType === 'audio' && (
                    ...
                    <div
                      style={{ textAlign: 'center', color: 'var(--pp-text-3)', padding: '20px' }}
                    >
        ...
        <div
          className="pp-card pp-table-scroll"
          style={{ padding: 0, borderRadius: '12px', border: '1px solid #99f6e4' }}
        >
          <div
            style={{
              padding: '12px 16px',
          ...
          >
            ...
            <span
              style={{ fontSize: '0.72rem', color: '#5eead4', fontWeight: 600, marginLeft: '4px' }}
            >
              ({images.length})
            </span>
          </div>
              ...
              {groupedImages.map((group) => (
                <React.Fragment key={group.date}>
                  {group.items.map((img: any, idx: number) => {
                    ...
                    const dateVal =
                      img.createdAt || img.created_at || img.recordedAt || img.recorded_at || 0;
                    const imagePath = img.picture || img.picturePath || img.picture_path;
                    const resolvedUrl = imagePath ? getImageUrl(imagePath) : '';
                    const mediaType = getMediaType(imagePath || '');
                          ...
                          >
                            {resolvedUrl ? (
                              <>
                                ...
                                {mediaType === 'video' && (
                                  <Video size={20} style={{ color: 'var(--pp-blue)' }} />
                                )}
                                {mediaType === 'audio' && (
                                  <FileAudio size={20} style={{ color: '#7c3aed' }} />
                                )}
                                {mediaType === 'pdf' && (
        ...
        <ImageLightbox
          ...
          onPrev={() =>
            setActivePreviewIndex(
              (activePreviewIndex - 1 + imageOnlyList.length) % imageOnlyList.length,
            )
          }
...
function ImageLightbox({
  ...
  const timestamp =
    ...
    activeImage.recordedAt ||
    activeImage.recorded_at;
    ...
    >
      ...
      >
        ...
        <div onClick={(e) => e.stopPropagation()} style={{ zIndex: 10 }}>
          {images.length > 1 && (
            <button className="lightbox-btn" onClick={onPrev} title="Previous (Left Arrow)">
              <ChevronLeft size={24} />
            </button>
          )}
        </div>
        ...
        <div onClick={(e) => e.stopPropagation()} style={{ zIndex: 10 }}>
          {images.length > 1 && (
            <button className="lightbox-btn" onClick={onNext} title="Next (Right Arrow)">
...
function DiagnosisView({
  ...
  const sortedSoap = soapRecords
    ? [...soapRecords].sort((a, b) => {
  ...
  const handleCopyToFollowup = (record: any) => {
    ...
    onAppendNote(
      `DIAGNOSIS (${new Date(record.createdAt).toLocaleDateString()}): ${parts.join(' | ')}`,
    );
  };
  ...
  const {
    expandedDates,
    toggleDate,
    groupedData: groupedSoap,
  } = useTableGrouping(currentSoap, 'createdAt');
                      ...
                      return (
                        ...
                        >
                          ...
                          <td>
                            ...
                            >
                              {record.assessment || 'No Diagnosis'}
                            </div>
                          ...
                          <td>
                            <div style={{ fontSize: '0.85rem', color: 'var(--pp-text-2)' }}>
                              {record.plan || record.advice || '—'}
                            </div>
                          </td>
                          <td style={{ textAlign: 'center' }}>
