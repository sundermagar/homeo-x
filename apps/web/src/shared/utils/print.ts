...
export const printBillingReport = (
  bills: any[], 
  org: any, 
  summary: { totalCharges: number; totalReceived: number; totalBalance: number; date: string }
) => {
  const formattedDate = format(new Date(summary.date), 'PPPP');
  ...
  const billRows = bills.map(b => `
    <tr>
      ...
      <td class="right">₹${b.charges.toLocaleString()}</td>
      <td class="right">₹${b.received.toLocaleString()}</td>
      <td class="right" style="color: ${b.balance > 0 ? '#e11d48' : '#1e293b'}">₹${b.balance.toLocaleString()}</td>
    </tr>
  `).join('');
  ...
  const html = `
    <html>
      <head>
        <title>Daily Billing Report - ${summary.date}</title>
        <style>
          @import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;600;700;800&display=swap');
          * { margin:0; padding:0; box-sizing:border-box; font-family: 'Inter', sans-serif; }
          body { padding: 40px; color: #1e293b; line-height: 1.5; font-size: 12px; }
          .container { max-width: 900px; margin: auto; }
          
          .letterhead { position: relative; padding: 14px 4px 10px; margin-bottom: 25px; border-bottom: 1px solid #e2e8f0; }
          .letterhead-band { position: absolute; top: 0; left: 0; right: 0; height: 6px; background: linear-gradient(90deg, #2563EB 0%, #60A5FA 70%, #BFDBFE 100%); border-radius: 0 0 2px 2px; }
          .letterhead-row { display: flex; align-items: center; gap: 14px; padding-top: 14px; }
          .letterhead-logo { width: 64px; height: 64px; object-fit: contain; border-radius: 8px; flex-shrink: 0; }
          .letterhead-logo-fallback { width: 64px; height: 64px; border-radius: 8px; background: #2563EB; color: #fff; display: flex; align-items: center; justify-content: center; font-size: 28px; font-weight: 800; flex-shrink: 0; }
          ...
          .report-title { display: inline-block; padding: 6px 20px; background: #0f172a; color: #fff; border-radius: 100px; font-size: 11px; font-weight: 800; text-transform: uppercase; letter-spacing: 0.1em; }
          .report-date { margin-top: 8px; font-size: 14px; font-weight: 700; color: #64748b; }
          
          .summary-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px; margin-bottom: 30px; }
          .summary-card { padding: 15px; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 12px; text-align: center; }
          .summary-label { font-size: 10px; font-weight: 800; color: #94a3b8; text-transform: uppercase; margin-bottom: 4px; }
          .summary-value { font-size: 20px; font-weight: 800; color: #0f172a; }

          table { width: 100%; border-collapse: collapse; margin-bottom: 30px; }
          th { text-align: left; padding: 10px; background: #f8fafc; border-bottom: 2px solid #e2e8f0; font-size: 10px; font-weight: 800; color: #94a3b8; text-transform: uppercase; }
          td { padding: 10px; border-bottom: 1px solid #f1f5f9; font-weight: 500; }
          .right { text-align: right; }
          
          .footer { margin-top: 60px; display: flex; justify-content: space-between; align-items: flex-end; border-top: 1px solid #f1f5f9; padding-top: 30px; }
          .footer-text { font-size: 10px; color: #94a3b8; font-weight: 600; }
          .sig-box { text-align: center; }
          .sig-line { width: 150px; border-top: 1px solid #cbd5e1; margin-bottom: 5px; }
      ...
      <body>
        <div class="container">
          <div class="no-print" style="margin-bottom: 20px; text-align: right;">
            <button onclick="window.print()" style="padding: 10px 20px; background: #2563EB; color: #fff; border: none; border-radius: 8px; cursor: pointer; font-weight: 800;">Confirm Print Report</button>
          </div>
          ...
          <div class="letterhead">
            ...
            <div class="letterhead-row">
              ${org.logoUrl 
                ? `<img src="${org.logoUrl}" alt="" class="letterhead-logo" />` 
                : `<div class="letterhead-logo-fallback">${(org.name || 'C').charAt(0).toUpperCase()}</div>`
              }
              <div class="letterhead-title">
                <div class="clinic-name">${org.name}</div>
          ...
          <div class="report-header">
            <span class="report-title">Daily Billing & Collection Report</span>
            <p class="report-date">${formattedDate}</p>
          </div>

          <div class="summary-grid">
            <div class="summary-card">
              <p class="summary-label">Total Billings</p>
              <p class="summary-value">₹${summary.totalCharges.toLocaleString()}</p>
            </div>
            <div class="summary-card" style="border-color: #22c55e20; background: #f0fdf4;">
              <p class="summary-label" style="color: #16a34a;">Net Collection</p>
              <p class="summary-value" style="color: #16a34a;">₹${summary.totalReceived.toLocaleString()}</p>
            </div>
            <div class="summary-card" style="border-color: #ef444420; background: #fef2f2;">
              <p class="summary-label" style="color: #dc2626;">Outstanding</p>
              <p class="summary-value" style="color: #dc2626;">₹${summary.totalBalance.toLocaleString()}</p>
            </div>
          </div>
          ...
          <div class="footer">
            <div class="footer-text">
              ...
              Clinical Records · HomeoX Management Suite
            </div>
            <div class="sig-box">
              <div class="sig-line"></div>
