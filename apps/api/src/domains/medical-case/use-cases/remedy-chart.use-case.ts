...
export class RemedyChartUseCase {
  ...
  async getRemedyLookups(): Promise<RemedyLookups> {
    const [medRows, potRows, freqRows] = await Promise.all([
      ...
      this._executeWithFallback(
        ...
        () =>
          this.db.execute(
            sql`SELECT id, name FROM potencies1 WHERE deleted_at IS NULL ORDER BY id ASC`,
          ),
      ),
      this._executeWithFallback(
        () =>
  ...
  async savePrescription(dto: SavePrescriptionDto): Promise<{ id: number }> {
    ...
    if (dto.id) {
      const existing = await this.db.execute(
        sql`SELECT rand_id FROM case_potencies WHERE id = ${dto.id}`,
      );
    ...
    if (deliveryMode && syncRandId) {
      if (deliveryMode === 'courier' || deliveryMode === 'pickup') {
        ...
        const check = await this.db.execute(
          sql`SELECT id FROM courier_medicine WHERE rand_id = ${syncRandId}`,
        );
  ...
  async deletePrescription(id: number): Promise<void> {
    ...
    console.log(
      `🗑️ Last prescription for regid=${regid} date=${dateval} deleted, cascading cleanup...`,
    );
