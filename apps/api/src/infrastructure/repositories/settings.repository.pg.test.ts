...
describe('SettingsRepositoryPg', () => {
  ...
  it('creates department using the correct SQL and params', async () => {
    const q1Spy = vi
      .spyOn(repo as any, 'q1')
      .mockResolvedValue({ id: 1, name: 'Test', description: null, isActive: true });

    await repo.createDepartment({ name: 'Test' });

    expect(q1Spy).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO departments'), [
      'Test',
      null,
      true,
    ]);
  ...
  it('updates department using the correct SQL and params', async () => {
    const q1Spy = vi
      .spyOn(repo as any, 'q1')
      .mockResolvedValue({ id: 2, name: 'Updated', description: 'New', isActive: false });

    await repo.updateDepartment(2, { name: 'Updated', description: 'New', isActive: false });

    expect(q1Spy).toHaveBeenCalledWith(expect.stringContaining('UPDATE departments SET'), [
      'Updated',
      'New',
      false,
      2,
  ...
  it('clears default PDF settings before inserting a default config', async () => {
    ...
    const q1Spy = vi
      .spyOn(repo as any, 'q1')
      .mockResolvedValue({ id: 3, templateName: 'Report', isDefault: true });
    ...
    expect(q1Spy).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO pdf_settings'), [
      'Report',
      null,
      null,
      null,
