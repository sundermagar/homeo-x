...
export class PackageRepositoryPG implements PackageRepository {
  ...
  async getActivePackage(regid: number): Promise<PatientPackage | null> {
    ...
    return {
      ...row.pp,
      packageName: row.planName ?? undefined,
      packagePrice: row.planPrice ?? undefined,
  ...
  async getRevenueStats(): Promise<{
    ...
    const [active] = await this.db
      .select({
        ...
        total: sql<number>`coalesce(sum(${schema.packagePlans.price}), 0)`,
      })
      .from(schema.patientPackages)
      .leftJoin(schema.packagePlans, eq(schema.patientPackages.packageId, schema.packagePlans.id))
      .where(
    ...
    const [expired] = await this.db
      ...
      .from(schema.patientPackages)
      .where(
        and(eq(schema.patientPackages.status, 'Expired'), isNull(schema.patientPackages.deletedAt)),
      );
