...
import type {
  ...
  ProcedureCode,
  SnomedConcept,
} from '../../domains/clinical-terminology/ports/terminology.repository.js';
...
export class TerminologyRepositoryPg implements ITerminologyRepository {
  ...
  async searchIcdCodes(query: string, limit = 20): Promise<IcdCode[]> {
    ...
    const results = await this.db
      .select()
      .from(icdCodes)
      .where(or(ilike(icdCodes.code, q), ilike(icdCodes.description, q)))
      .limit(limit);
    return results;
  }
  ...
  async searchSnomedConcepts(query: string, limit = 20): Promise<SnomedConcept[]> {
    ...
    try {
      ...
      const results = await this.db
        .select()
        .from(snomedConcepts)
        .where(or(ilike(snomedConcepts.term, q), ilike(snomedConcepts.fsn, q)))
        .limit(limit);
  ...
  async getIcdByCode(code: string): Promise<IcdCode | undefined> {
    const results = await this.db
      .select()
      .from(icdCodes)
      .where(ilike(icdCodes.code, code))
      .limit(1);
    return results[0];
  }
  ...
  async getLoincByNum(loincNum: string): Promise<LoincCode | undefined> {
    const results = await this.db
      .select()
      .from(loincCodes)
      .where(ilike(loincCodes.loincNum, loincNum))
      .limit(1);
    return results[0];
  }
  ...
  async getProcedureByCode(code: string): Promise<ProcedureCode | undefined> {
    const results = await this.db
      .select()
      .from(procedureCodes)
      .where(ilike(procedureCodes.code, code))
      .limit(1);
    return results[0];
  }
  ...
  async getSnomedByConceptId(conceptId: string): Promise<SnomedConcept | undefined> {
    ...
    const results = await this.db
      .select()
      .from(snomedConcepts)
      .where(sql`${snomedConcepts.conceptId} = ${id}`)
      .limit(1);
