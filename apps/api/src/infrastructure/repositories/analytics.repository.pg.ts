...
export class AnalyticsRepositoryPg implements IAnalyticsRepository {
  ...
  async getSummary(): Promise<AnalyticsSummary> {
    const [[patients], [appointments], [revenue]] = await Promise.all([
      this.db.select({ total: sql<number>`count(*)::int` }).from(schema.patients).where(sql`(deleted_at IS NULL OR deleted_at::text = '')`),
      this.db.select({ total: sql<number>`count(*)::int` }).from(schema.appointments).where(sql`(deleted_at IS NULL OR deleted_at::text = '')`),
      this.db.select({ total: sql<number>`COALESCE(sum(CAST(NULLIF(amount, '') AS numeric)), 0)::int` }).from(schema.receiptLegacy).where(sql`(deleted_at IS NULL OR deleted_at::text = '')`),
    ]);
  ...
  async getPatientTrends(from?: Date, to?: Date): Promise<PatientTrendResult> {
    ...
    // New Patients
    const newPatients = await this.db.execute(sql`
    ...
    // Revenue
    const revenueByMonth = await this.db.execute(sql`
    ...
    // Top Diagnoses (from homeoDetails or condition in medicalCases)
    const topDiagnoses = await this.db.execute(sql`
    ...
    return {
      newPatients: newPatients as any,
      revenueByMonth: revenueByMonth as any,
      topDiagnoses: topDiagnoses as any,
    };
  }
  ...
  async getMonthWiseBreakdown(fromYearMth: string, toYearMth: string): Promise<MonthWiseResult[]> {
    ...
    for (const m of months) {
      ...
      const [newCases] = await this.db.execute(sql`SELECT count(*)::int as cnt FROM medicalcases WHERE created_at::date BETWEEN ${firstDay} AND ${lastDay} AND (deleted_at IS NULL OR deleted_at::text = '')`);
      const [followups] = await this.db.execute(sql`SELECT count(*)::int as cnt FROM case_potencies WHERE dateval::date BETWEEN ${firstDay} AND ${lastDay} AND (deleted_at IS NULL OR deleted_at::text = '')`);

      const receipts = await this.db.execute(sql`
      ...
      results.push({
        ...
        displaydate,
        new_cases: Number((newCases as any)?.cnt) || 0,
        followups: Number((followups as any)?.cnt) || 0,
        collection: payments,
        cash, cheque, online, card,
        product_charges: 0, // Legacy feature rarely used
        expenses: Number((expenses as any)?.total) || 0,
        cash_deposit: 0,
        bank_deposit: 0,
        cash_in_hand: 0,
      });
