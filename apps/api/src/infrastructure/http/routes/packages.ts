...
const getPatientRepo = (req: any) => new MedicalCaseRepositoryPg(req.tenantDb);
const getCommRepo = (req: any) => new CommunicationRepositoryPG(req.tenantDb);
const smsGateway = createSmsGateway();
const getSmsUseCase = (req: any) => new SendSmsUseCase(getCommRepo(req), smsGateway);
...
packagesRouter.get(
  '/patient/:regid',
  asyncHandler(async (req, res) => {
    const uc = new AssignPackageUseCase(
      getRepo(req),
      getBillingRepo(req),
      getPatientRepo(req),
      getSmsUseCase(req),
...
packagesRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!name || price == null || !durationDays)
      throw new BadRequestError('name, price, and durationDays are required');
    const uc = new ManagePackagePlansUseCase(getRepo(req));
...
packagesRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!regid || !packageId || !patientId)
      throw new BadRequestError('regid, packageId, and patientId are required');
    const uc = new AssignPackageUseCase(
      getRepo(req),
      getBillingRepo(req),
      getPatientRepo(req),
