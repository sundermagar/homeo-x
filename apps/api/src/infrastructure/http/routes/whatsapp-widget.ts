...
whatsappWidgetRouter.get('/config/:channelId', asyncHandler(async (req, res) => {
  ...
  const config = typeof widget.widget_config === 'string' ? JSON.parse(widget.widget_config) : widget.widget_config;
  sendSuccess(res, { enabled: true, ...config });
}));

// GET /api/widget/qa/:channelId - Fetch FAQs
whatsappWidgetRouter.get('/qa/:channelId', asyncHandler(async (req, res) => {
  const channelId = Number(req.params.channelId);
  const { client } = await getDbForChannel(channelId);
...
whatsappWidgetRouter.post('/contacts', asyncHandler(async (req, res) => {
  ...
  if (!contact) {
    ...
    contact = await repo.saveContact({ ...contact, name: name || contact.name, email: email || contact.email });
  }
...
whatsappWidgetRouter.get('/conversation/:conversationId', asyncHandler(async (req, res) => {
