...
scribingRouter.post('/sessions', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!visitId) {
      res.status(400).json({ success: false, error: 'visitId required' });
      return;
    }
    ...
    const existing = await db
      .select()
      .from(schema.scribingSessions)
      .where(eq(schema.scribingSessions.visitId, visitId))
      .limit(1);
    if (existing[0]) {
    ...
    const [created] = await db
      .insert(schema.scribingSessions)
      .values({
        visitId,
...
scribingRouter.patch('/sessions/:sessionId/pause', async (req, res, next) => {
  try {
    sendSuccess(res, formatSession(await updateSessionStatus(req, 'PAUSED')));
  } catch (err) {
    next(err);
  }
});
...
scribingRouter.patch('/sessions/:sessionId/resume', async (req, res, next) => {
  try {
    sendSuccess(res, formatSession(await updateSessionStatus(req, 'ACTIVE')));
  } catch (err) {
    next(err);
  }
});
...
scribingRouter.patch('/sessions/:sessionId/end', async (req, res, next) => {
  try {
    sendSuccess(res, formatSession(await updateSessionStatus(req, 'COMPLETED')));
  } catch (err) {
    next(err);
  }
});
...
scribingRouter.patch(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (typeof req.body?.totalDurationMs === 'number')
        patch.totalDurationMs = req.body.totalDurationMs;
...
scribingRouter.post(
  '/sessions/:sessionId/generate-soap',
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const uc = getConsultationUseCase();
      const result = await uc.suggestSoap(getTenant(req), getUserId(req), {
