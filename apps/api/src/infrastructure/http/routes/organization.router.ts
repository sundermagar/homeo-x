  ...
  router.post(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      if (dbUrl) {
        ...
        (async () => {
          try {
            const { provisionTenant, migrateTenant, TenantRegistry, createDbClient } =
            ...
            if (shouldProvision) {
              logger.info(
                { organizationName: result.name, schemaName },
                'Starting background provisioning...',
              );
              ...
              try {
                ...
                logger.warn(
                  { err: tenantAdminErr.message, schemaName },
                  'Tenant admin creation failed in background',
                );
              }
              ...
              logger.info(
                { organizationName: result.name, slug, schemaName },
                'Tenant provisioned and registered in background',
              );
            }
            ...
            if (req.body.adminEmail && req.body.adminPassword) {
              try {
                ...
                `)) as any[];
                  ...
                  `);
                    logger.info(
                      ...
                      'Public schema clinic admin mirrored successfully in background',
                    );
                  }
                }
              } catch (publicErr: any) {
                logger.error(
                  ...
                  'Failed to mirror admin to public schema in background',
                );
              }
            }
            ...
            logger.error(
              { err: bgErr.message, organizationName: result.name },
              'Fatal background provisioning error',
            );
          }
        })();
      }
