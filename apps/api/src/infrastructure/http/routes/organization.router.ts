  ...
  router.post('/', validate(createOrganizationSchema), asyncHandler(async (req: Request, res: Response) => {
    ...
    if (dbUrl) {
      ...
      void (async () => {
        ...
        if (shouldProvision) {
          logger.info({ organizationName: result.name, schemaName }, 'Starting synchronous provisioning...');
          ...
          try {
            ...
            logger.warn({ err: tenantAdminErr.message, schemaName }, 'Tenant admin creation failed');
          }
          ...
          try {
            ...
            if (!existing || existing.length === 0) {
              ...
              if (userId) {
                ...
                `);
                logger.info({ adminEmail: req.body.adminEmail }, 'Public schema clinic admin mirrored successfully');
              }
            }
          } catch (publicErr: any) {
