  ...
  router.post(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      if (dbUrl) {
        ...
        const { TenantRegistry } = await import('@mmc/database');
        ...
        if (shouldProvision) {
          ...
          logger.info(
            ...
            'Tenant registered synchronously',
          );
        }
        ...
        if (req.body.adminEmail && req.body.adminPassword) {
          try {
            ...
            logger.error({ err: publicErr.message }, 'Failed to mirror admin to public schema');
          }
        }
        ...
        void (async () => {
          try {
            ...
            logger.info(
              { organizationName: result.name, slug, schemaName },
              'Tenant background provisioning complete',
            );
