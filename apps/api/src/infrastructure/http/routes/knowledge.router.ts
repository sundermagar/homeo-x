...
router.get('/dictionary', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const rows = await pgQuery(
      req.tenantDb,
      'SELECT * FROM dictionary WHERE deleted_at IS NULL ORDER BY title ASC',
    );
...
router.get('/dictionary/:id', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const row = await pgQueryOne(
      req.tenantDb,
      'SELECT * FROM dictionary WHERE id = $1 AND deleted_at IS NULL LIMIT 1',
      [req.params.id],
    );
    if (!row)
      return res.status(404).json({ success: false, message: 'Dictionary entry not found' });
...
router.delete('/dictionary/:id', async (req: Request, res: Response, next: NextFunction) => {
  try {
    await pgQuery(req.tenantDb, 'UPDATE dictionary SET deleted_at = NOW() WHERE id = $1', [
      req.params.id,
    ]);
...
router.get('/books', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const rows = await pgQuery(
      req.tenantDb,
      'SELECT * FROM library_resources WHERE deleted_at IS NULL ORDER BY title ASC',
    );
    res.json({ success: true, data: rows });
  } catch (error) {
    next(error);
...
router.get('/books/:id', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const row = await pgQueryOne(
      req.tenantDb,
      'SELECT * FROM library_resources WHERE id = $1 AND deleted_at IS NULL LIMIT 1',
      [req.params.id],
    );
...
router.delete('/books/:id', async (req: Request, res: Response, next: NextFunction) => {
  try {
    await pgQuery(req.tenantDb, 'UPDATE library_resources SET deleted_at = NOW() WHERE id = $1', [
      req.params.id,
    ]);
