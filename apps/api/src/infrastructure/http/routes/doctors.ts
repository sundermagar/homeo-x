...
doctorsRouter.get('/', authMiddleware, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const activeUsersRows = await req.publicDb.execute(
      sql`SELECT id, email, is_active FROM public.users WHERE LOWER(type) IN ('doctor', 'medical practitioner')`,
    );
    ...
    logger.info({ 
      userContextId: req.user?.contextId, 
      findAllResultCount: result.data.length,
      activeUsersCount: statusMap.size,
      mappedDoctorsCount: doctors.length,
      doctors
    }, 'Doctors API debug');
...
doctorsRouter.patch(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    `);
      ...
      logger.info(
        { userId: user.id, isActive, rowCount: (result as any).rowCount || (result as any).length },
        'Doctor status updated in public.users',
      );
