...
function verifyWebhookSignature(req: any): boolean {
  ...
  if (!appSecret) {
    ...
    logger.warn(
      'WHATSAPP_APP_SECRET not set — webhook signature verification skipped. Set it for production!',
    );
    return true;
  }
...
//   const challenge = req.query['hub.challenge'];
  ...
  const expectedToken =
    process.env.WHATSAPP_WEBHOOK_VERIFY_TOKEN || process.env.VERIFY_TOKEN || 'kreed_verify_token';
...
whatsappRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const msgType =
      req.body?.entry?.[0]?.changes?.[0]?.value?.messages?.[0]?.type || 'status_update';
    const phoneId =
      req.body?.entry?.[0]?.changes?.[0]?.value?.metadata?.phone_number_id || 'unknown';
    ...
    try {
      ...
      `);
        ...
        if (phoneNumberId) {
          logger.info(
            `Webhook lookup: Scanning ${activeSchemas.length} active schemas for wa_channels matching phone_number_id: ${phoneNumberId}`,
          );
        ...
        if (!matchedSchemaName && (statusId || messageId)) {
          ...
          logger.info(
            `Webhook lookup: Scanning ${activeSchemas.length} active schemas for wa_messages matching message id: ${queryId}`,
          );
          for (const schemaName of activeSchemas) {
            try {
              const client = getCachedDbClient(schemaName);
        ...
        if (matchedSchemaName) {
          ...
          logger.warn(
            `Webhook lookup: No matching channel or message found in any tenant schema for event. Falling back to default.`,
          );
        }
      }
    } catch (err: any) {
...
whatsappRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    sendSuccess(
      res,
      result,
      `Synced ${result.total} templates: ${result.created} new, ${result.updated} updated`,
    );
  }),
);
...
whatsappRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (mediaId) {
      result = await gateway.sendMedia(
    ...
    if (result.success) {
      const displayContent = content
        ? mediaId
          ? `${content} (Attachment: ${fileName || mediaType || 'file'})`
          : content
...
whatsappRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!message || !message.whatsappMessageId)
      throw new BadRequestError('Message not found or missing WAMID');

    const conversation = await repo.findConversationById(message.conversationId);
    if (!conversation) throw new BadRequestError('Conversation not found');

    const gateway = getGateway(req);
    const result = await gateway.sendReaction(
      conversation.channelId,
      conversation.contactPhone,
  ...
  asyncHandler(async (req, res) => {
    ...
    if (idStr.startsWith('patient_')) {
      throw new BadRequestError(
        'Core patient directory entries cannot be deleted from the WhatsApp contact list.',
      );
    }
    const repo = getRepo(req);
...
whatsappRouter.post(
  ...
  authMiddleware,
  asyncHandler(async (req, res) => {
    const data = await getRepo(req).saveTrainingData({
      ...req.body,
    ...
    if (!channelId) {
      ...
      if (!defaultChannel) {
        if (!process.env.WHATSAPP_TOKEN) {
          throw new BadRequestError(
            'No active WhatsApp channel configured. Please add a WABA channel first or set WHATSAPP_TOKEN in .env.',
          );
        }
      } else {
...
whatsappRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      const templateRows = await (repo as any).db
        .select()
        .from(waTemplates)
        .where(
          and(
      ...
      if (templateRows.length > 0) {
        ...
        if (tpl.whatsappTemplateId && String(tpl.whatsappTemplateId).startsWith('local_')) {
          throw new BadRequestError(
            `This template ('${templateName}') exists only locally in your database. To send template messages via WhatsApp, you must first register and get this template approved in your Meta WhatsApp Business Account (WABA) Manager under the name '${templateName}' with '${language || 'en_US'}' language, and then click 'Sync Templates' to import it.`,
          );
        }
      ...
      logger.warn(
        { err: e.message },
        'Failed to pre-fetch WABA template from DB for header injection',
      );
    }
...
whatsappRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    sendSuccess(res, {
      configured: !!channel,
      channel: channel
        ? { id: channel.id, name: channel.name, phoneNumber: channel.phoneNumber }
        : null,
    });
  }),
);
...
whatsappRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (mimeType === 'application/pdf') {
      ...
      throw new BadRequestError(
        'Unsupported file type. Supported formats are PDF, TXT, CSV, and MD.',
      );
    }
    ...
    const id = saved.id;
    import('../../../domains/whatsapp/services/training.service.js').then(
      async ({ processTrainingSource }) => {
        try {
          await processTrainingSource(repo, id);
        } catch (err: any) {
          logger.error(
            `Background training source processing failed for source ${id}: ${err.message}`,
          );
        }
      },
    );
...
whatsappRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const dictionaryQuery = await db.execute(
...
whatsappRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!aiSetting || !aiSetting.apiKey) {
      sendSuccess(res, {
        response:
          'AI Agent is not configured. Please enter your OpenAI/Provider API key in the AI Settings.',
    ...
    try {
      const { searchTrainingData } =
        await import('../../../domains/whatsapp/services/training.service.js');
