    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      if (dbUrl && !result.clinicId) {
        try {
          ...
          if (shouldProvision) {
            ...
            logger.info(
              { baseName, slug, schemaName },
              'Successfully provisioned new tenant database schema',
            );
          }
        } catch (err: any) {
      ...
      if (req.body.sendWelcomeEmail && req.body.email && req.body.password) {
        try {
          ...
          logger.error(
            { err: emailErr.message, email: req.body.email },
            'Failed to send account welcome email',
          );
        }
      }
      ...
      res
        .status(201)
        .json({ success: true, message: 'Account created successfully', data: result });
    }),
  );
