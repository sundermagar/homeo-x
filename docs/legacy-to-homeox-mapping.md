# Legacy MMC -> MMC Table Mapping

This document maps legacy MySQL tables (MMC) to current MMC PostgreSQL tables and provides a safe migration sequence.

## Scope

- Source: legacy tables loaded into PostgreSQL tenant schema (example: `tenant_demo`)
- Target: MMC app tables from `packages/database/src/schema/*.ts`
- Goal: move data from legacy-compatible shape to app-consumable shape

## Current MMC Table Names (as implemented)
