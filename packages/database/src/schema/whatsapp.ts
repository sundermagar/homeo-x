...
export const waRecipientStatusEnum = pgEnum('wa_recipient_status', [
  'pending',
  'sent',
  'delivered',
  'read',
...
export const waAutomationStatusEnum = pgEnum('wa_automation_status', [
  'active',
  'inactive',
  'paused',
]);
...
export const waTemplates = pgTable(
  ...
  (table) => ({
    templateChannelWaIdUnique: unique('wa_template_channel_wa_id_unique').on(
      table.whatsappTemplateId,
      table.channelId,
    ),
...
// ─── Campaign Recipients ────────────────────────────────────────────────────
export const waCampaignRecipients = pgTable(
...
export const waMessages = pgTable(
  ...
  {
    id: serial('id').primaryKey(),
    conversationId: integer('conversation_id').references(() => waConversations.id, {
      onDelete: 'cascade',
    }),
    whatsappMessageId: varchar('whatsapp_message_id', { length: 255 }),
...
export const waContacts = pgTable(
  ...
  (table) => ({
    contactPhoneClinicUnique: unique('wa_contact_phone_clinic_unique').on(
      table.phone,
      table.clinicId,
    ),
  }),
...
export const waAiSettings = pgTable(
  'wa_ai_settings',
  {
    id: serial('id').primaryKey(),
    channelId: integer('channel_id').references(() => waChannels.id, { onDelete: 'cascade' }),
...
export const waTrainingSources = pgTable(
  'wa_training_sources',
  {
    id: serial('id').primaryKey(),
    channelId: integer('channel_id').references(() => waChannels.id, { onDelete: 'cascade' }),
...
export const waTrainingChunks = pgTable(
  'wa_training_chunks',
  {
    id: serial('id').primaryKey(),
    sourceId: integer('source_id')
      .notNull()
      .references(() => waTrainingSources.id, { onDelete: 'cascade' }),
    channelId: integer('channel_id').references(() => waChannels.id, { onDelete: 'cascade' }),
    content: text('content').notNull(),
...
export const waTrainingQaPairs = pgTable(
  'wa_training_qa_pairs',
  {
    id: serial('id').primaryKey(),
    channelId: integer('channel_id').references(() => waChannels.id, { onDelete: 'cascade' }),
