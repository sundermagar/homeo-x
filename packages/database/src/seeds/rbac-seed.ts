...
async function repairRbacSchema(db: DbClient) {
  ...
  try {
    ...
    await db.execute(
      sql`ALTER TABLE permissions ADD COLUMN IF NOT EXISTS display_name TEXT DEFAULT ''`,
    );
    ...
    try {
      await db.execute(
        sql`ALTER TABLE permissions ADD CONSTRAINT permissions_slug_key UNIQUE (slug)`,
      );
    } catch (e) {
...
export async function seedRbac(db: DbClient) {
  ...
  const permissions = [
    {
      ...
      description: 'Grants access to view patient lists and search.',
    },
    {
      id: 2,
      name: 'Write Patients',
      slug: 'PATIENT_WRITE',
      module: 'Clinical',
      displayName: 'Create/Edit Patients',
      description: 'Grants access to register and update patient profiles.',
    },
    {
      id: 3,
      ...
      description: 'Grants access to soft-delete patient records.',
    },

    {
      ...
      description: 'Grants access to view invoices and collection reports.',
    },
    {
      id: 11,
      name: 'Write Billing',
      ...
      description: 'Grants access to generate new bills and receipts.',
    },

    {
      ...
      description: 'Grants access to add or edit staff members (Doctors, Receptionists).',
    },
    {
      id: 21,
      name: 'Manage RBAC',
      ...
      description: 'Grants access to modify roles and permissions matrix.',
    },

    {
      ...
      slug: 'STOCKS_VIEW',
      module: 'Inventory',
      displayName: 'View Stocks',
      description: 'Grants access to view medicine inventory levels.',
    },
    {
      id: 31,
      name: 'Manage Stocks',
      ...
      description: 'Grants access to update medicine inventory and stock logs.',
    },
  ];
  ...
  for (const p of permissions) {
    const existingById = await db
      .select()
      .from(permissionsLegacy)
      .where(eq(permissionsLegacy.id, p.id))
      .limit(1);
    const existingBySlug = await db
      .select()
      .from(permissionsLegacy)
      .where(eq(permissionsLegacy.slug, p.slug))
      .limit(1);
  ...
  const roles = [
    ...
    },
    {
      ...
      description: 'Access to clinical and patient data.',
      parent: 1,
      dept: 1,
    },
    {
      ...
      description: 'Access to patient intake and scheduling.',
      parent: 2,
      dept: 1,
    },
    {
  ...
  for (const r of roles) {
    const existingById = await db
      .select()
      .from(rolesLegacy)
      .where(eq(rolesLegacy.id, r.id))
      .limit(1);
    const existingByName = await db
      .select()
      .from(rolesLegacy)
      .where(eq(rolesLegacy.name, r.name))
      .limit(1);
  ...
  const assignments = [
    ...
    ...permissions
      .filter((p) => p.slug !== 'RBAC_MANAGE')
      .map((p) => ({ roleId: 2, permissionId: p.id })),
