...
async function main() {
  ...
  for (const tenant of tenants) {
    try {
      console.log(
        `[Seed:FAQ] Seeding FAQs for tenant: ${tenant.displayName} (${tenant.schemaName})...`,
      );
      ...
      try {
        ...
        await db.execute(
          sql`ALTER TABLE faqs ADD COLUMN IF NOT EXISTS display_order integer DEFAULT 0`,
        );
        await db.execute(
          sql`ALTER TABLE faqs ADD COLUMN IF NOT EXISTS is_active boolean DEFAULT true`,
        );
        ...
        await db.execute(
          sql`ALTER TABLE faqs ALTER COLUMN created_at SET DEFAULT CURRENT_TIMESTAMP`,
        );
        await db.execute(
          sql`ALTER TABLE faqs ALTER COLUMN updated_at SET DEFAULT CURRENT_TIMESTAMP`,
        );
      } catch (patchErr) {
